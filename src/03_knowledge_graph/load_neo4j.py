...
# Recharger la configuration dynamiquement pour refléter les changements
import config_neo4j
importlib.reload(config_neo4j)
...
def load_offres(driver: Driver):
    ...
    for _, r in df_o.iterrows():
        ctx_rows.append({
            "oid":      r["offre_id"],
            "secteur":  r["secteur_principal"],
            "employeur":r["employeur"],
            "ville":    r["ville_principale"],
...
def load_candidats(driver: Driver):
    ...
    for _, r in df_c.iterrows():
        cand_rows.append({
            ...
            "diplome_raw":      r["diplome_raw"] if pd.notna(r["diplome_raw"]) else None,
            "ncf_niveau_final": (int(r["ncf_niveau_final"])
                                  if pd.notna(r["ncf_niveau_final"])
                                  else None),
            "qualification":    r["qualification_metier"] if pd.notna(r["qualification_metier"]) else None,
            "secteur_metier":   r["secteur_metier"] if pd.notna(r["secteur_metier"]) else None,
            "filiere":          r["filiere_specialite"] if pd.notna(r["filiere_specialite"]) else None,
            "secteur_demande":  r["secteur_demande"] if pd.notna(r["secteur_demande"]) else None,
            "metier_vise":      r["metier_vise"] if pd.notna(r["metier_vise"]) else None,
            "objectif":         str(r["objectif"] if pd.notna(r["objectif"]) else "")[:300],
            "mobilite_geo":     bool(r["mobilite_geo_bool"])
                                 if pd.notna(r["mobilite_geo_bool"])
                                 else None,
            "ncf_code_str":     (str(int(r["ncf_niveau_final"]))
                                  if pd.notna(r["ncf_niveau_final"])
                                  else None),
            "text_to_embed":    str(r["text_to_embed"] if pd.notna(r["text_to_embed"]) else "")[:400],
        })
