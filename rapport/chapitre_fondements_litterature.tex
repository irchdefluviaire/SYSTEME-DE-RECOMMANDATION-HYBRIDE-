...
\section{Repères conceptuels de l'étude}

\subsection{Vocabulaire opérationnel : du matching à l'Agentic RAG}
...
\medskip
\noindent\textbf{Base de données vectorielle.}

Une base de données vectorielle stocke des représentations numériques d'objets et permet de retrouver rapidement ceux dont les vecteurs sont les plus proches d'une requête. Elle sert donc à rechercher des textes similaires par leur sens, et non seulement par des mots identiques. Dans ce mémoire, cette fonction est mise en œuvre avec PostgreSQL et son extension pgvector \cite{pgvector2024}. Elle présélectionne les offres, métiers, compétences ou documents proches, sans décider seule de leur compatibilité professionnelle.
...
Cette typologie doit toutefois être actualisée. Les approches profondes de recommandation prolongent le filtrage par contenu en encodant les descriptions d'offres et de profils dans des représentations contextuelles. BERT fournit des représentations sensibles au contexte, tandis que Sentence-BERT rend ces représentations directement comparables pour la recherche par similarité \cite{devlin2018bert,reimers2019sbert}. Dans un domaine où les intitulés sont hétérogènes, cette couche sémantique peut rapprocher « analyste de données », « data analyst » et des descriptions de compétences voisines ; elle ne démontre cependant pas, à elle seule, la compatibilité professionnelle. Le tableau~\ref{tab:comparaison_approches_recommandation} compare ces familles dans le contexte de l'emploi.
\begin{table}[H]
\centering
\caption{Comparaison des principales approches de recommandation}
\label{tab:comparaison_approches_recommandation}
\tableStyleStart
\begin{tabularx}{\textwidth}{p{2.4cm}p{3.1cm}X X X}
\rowcolor{darkblue}
\tabheader{Approche} & \tabheader{Principe} & \tabheader{Avantages} & \tabheader{Inconvénients} & \tabheader{Limites dans cette étude} \\
\midrule
Par contenu & Compare les caractéristiques des profils et des offres. & Fonctionne sans historique de clics et permet d'expliquer les ressemblances. & Dépend de la qualité des descriptions et peut rester trop proche du profil initial. & Les formulations varient entre sources et certaines descriptions sont incomplètes. \\
Collaborative & Utilise les comportements passés d'utilisateurs semblables. & Peut découvrir des préférences qui ne sont pas écrites dans les profils. & Exige beaucoup d'interactions et gère mal les nouveaux utilisateurs ou les nouvelles offres. & Les historiques de clics, candidatures et recrutements sont insuffisants. \\
Sémantique profonde & Encode offres et profils par des représentations contextuelles (BERT et SBERT). & Dépasse les synonymes, variantes d'intitulés et proximités lexicales. & Sensible au domaine, aux données d'entraînement et à la qualité des labels. & Sert au rappel initial ; une similarité forte ne constitue pas une preuve de compatibilité. \\
Fondée sur la connaissance & Exploite ontologies, référentiels ou graphe métier--compétence--formation. & Rend les contraintes et les explications relationnelles explicites. & Dépend de la couverture, de la qualité et de la maintenance des relations. & Le graphe vérifie et explique le vivier ; il ne remplace pas les données manquantes. \\
Hybride & Combine plusieurs signaux : textes, relations et règles métier. & Compense les faiblesses d'une source et améliore l'explication. & Demande de calibrer les signaux et rend le système plus complexe. & Sa qualité dépend encore de la fiabilité des données et des relations du graphe. \\
\bottomrule
\end{tabularx}
\tableStyleEnd
\source{Nos travaux, synthèse de Pazzani et Billsus (2007), Koren et al. (2009), He et al. (2017) et Burke (2002).}
\end{table}
Les approches fondées sur la connaissance complètent ce signal en imposant une structure explicite : ontologies, référentiels et graphes relient métiers, compétences, formations et profils. Des travaux récents en ressources humaines illustrent cette direction : HRGraph combine extraction par \LLM{} et graphe de connaissances pour des tâches de recommandation d'emploi \cite{wasi2024hrgraph}, tandis que Yang et Shen articulent graphe de connaissances, GCN, apprentissage par renforcement et filtrage collaboratif profond pour la prédiction de compétences \cite{yang2025talentgraph}. Ces résultats ne sont pas transposables automatiquement au cas d'étude ; ils motivent plutôt une architecture qui sépare rappel sémantique, contrôle relationnel et décision humaine.
...
Cette spécificité change l'interprétation des scores. Dans le commerce électronique, une recommandation peut viser la probabilité de clic ou d'achat. Dans l'emploi, elle doit distinguer trois situations :
\begin{itemize}
    \item le candidat est immédiatement compatible avec le poste ;
    \item le candidat est proche du profil attendu, mais doit encore acquérir certaines compétences ;
    \item le candidat est trop éloigné du poste cible pour une recommandation immédiate.
\end{itemize}
La notion de \emph{skill gap} devient alors centrale.
...
\begin{figure}[H]
  ...
  \includegraphics[width=0.92\textwidth]{figures/evolution_llms.png}
  \source{Nos travaux, schématisation conceptuelle à partir de Mikolov et al. (2013)}
\end{figure}
...
Techniquement, un bi-encodeur transforme séparément la requête $q$ et un document $d$ en vecteurs $e(q)$ et $e(d)$. Après normalisation $L_2$, leur proximité est calculée par similarité cosinus :
\[
s_{sem}(q,d)=\frac{e(q)^\top e(d)}{\lVert e(q)\rVert_2\lVert e(d)\rVert_2}.
\]
Le moteur restitue un vivier top-$K$ plutôt qu'une décision finale. Pour conserver une latence praticable, ce vivier est retrouvé par recherche approximative de plus proches voisins : avec HNSW, les paramètres de construction $M$ et $ef_{construction}$ règlent la richesse de l'index, alors que $ef_{search}$ arbitre, à l'interrogation, entre rappel et temps de réponse \cite{malkov2018hnsw,johnson2019faiss}. Le fine-tuning contrastif est donc pertinent seulement s'il améliore le classement sur des paires emploi--profil tenues à l'écart de l'apprentissage.
...
\paragraph{Perspective d'apprentissage relationnel.}
La structure multi-relationnelle du projet constitue néanmoins un support pertinent pour une \GNN{} hétérogène à moyen terme. Un tel modèle apprend la représentation d'une entité à partir de ses voisins et du type de lien qui les relie ; une architecture relationnelle de type R-GCN est donc plus cohérente qu'un modèle limité aux seules interactions bipartites candidat--offre \cite{schlichtkrull2018rgcn}. Lorsque la plateforme disposera d'interactions horodatées et validées (consultation, candidature, entretien, recrutement ou validation de compétences), ce modèle pourra être évalué pour prédire la pertinence d'un lien candidat--offre, compléter des compétences manquantes ou enrichir le reranking. Des travaux tels que JobXMLC montrent l'intérêt de cette piste pour la prédiction de compétences dans un graphe emploi--compétences, sans pour autant garantir sa transposition au contexte camerounais \cite{goyal2023jobxmlc}.

Cette piste reste une perspective et non un résultat du présent mémoire. En l'absence de labels d'interaction indépendants, une \GNN{} risquerait surtout de reproduire les relations déjà induites par l'extraction et d'amplifier le biais des noeuds très connectés. Son apport devra donc être comparé, sur un jeu de test séparé dans le temps et annoté par des professionnels, au score hybride actuel. Le graphe Neo4j demeure ainsi, dans l'architecture proposée, le support explicite de contrôle et d'explication des recommandations.
...
Dans le pipeline proposé, cette exigence se traduit par un état explicitement typé : requête, intention, plan d'outils, résultats de recherche, preuves, réponse, score du critic et trace. Les transitions sont bornées : après un top-$K$ vectoriel, le graphe vérifie les relations autorisées ; une requête Text2Cypher est limitée au schéma et à la lecture seule ; le critic ne peut ouvrir qu'une révision limitée avant l'acceptation ou le refus de répondre. Cette conception distingue le rôle du \LLM{} -- interpréter ou formuler -- de celui des sources, qui portent les preuves. Elle limite les boucles incontrôlées et rend chaque recommandation audit-able.
...
Cette métrique répond à la question suivante : parmi les premières recommandations affichées, combien sont réellement utiles ? Dans ce mémoire, Precision@10 porte sur les dix offres placées en tête pour chaque candidat. Sa limite est qu'elle ne mesure pas si le système a retrouvé tous les bons résultats disponibles.
...
Dans cette étude, la métrique principale retenue pour comparer et calibrer le reranking est donc \textbf{NDCG@10}. Elle récompense simultanément la pertinence graduée et le bon ordre des dix premières offres, qui constituent la zone effectivement consultée par l'utilisateur. Recall@10 est la métrique de garde-fou : un gain de NDCG ne serait pas acceptable s'il faisait disparaître une grande part des offres pertinentes. Precision@10 et MRR@10 restent secondaires, respectivement pour contrôler la qualité moyenne de la liste et la position de la première offre pertinente \cite{thakur2021beir,muennighoff2022mteb}. Ces mesures sont interprétées comme des diagnostics internes tant qu'aucun jugement métier indépendant n'est disponible.
...
Les travaux présentés dans cette revue montrent que le matching emploi-compétences ne peut pas être traité comme une simple recherche de mots-clés. Il exige à la fois une représentation sémantique des textes, une structuration des relations métier-compétence-formation et un mécanisme de génération capable d'expliquer les résultats sans s'éloigner des données disponibles. Cette étude se positionne à l'intersection de trois familles d'approches. La première est la recherche sémantique par embeddings, utilisée pour retrouver rapidement les offres, compétences ou documents proches d'une requête. La deuxième est le graphe de connaissances, mobilisé pour représenter les relations entre candidats, offres, compétences, métiers, secteurs et niveaux de formation. La troisième est l'orchestration agentique, qui organise le retrieval, l'enrichissement par le graphe, la génération et le contrôle de la réponse. La contribution du mémoire n'est donc pas de proposer un nouveau modèle de langage ou un nouvel algorithme de recommandation générique. Elle consiste à concevoir et évaluer une architecture hybride adaptée au contexte emploi-compétences : pgvector sert à la présélection sémantique, Neo4j apporte le signal relationnel et LangGraph orchestre les étapes de décision. Ce positionnement répond à une limite de la littérature : les approches générales de RAG ou de recommandation expliquent rarement comment articuler, dans un même système opérationnel, données locales, référentiels métiers, graphe, scoring hybride et contrôle de la génération. Le système proposé reste un outil d'aide à la décision. Il ne remplace ni le recruteur, ni le conseiller emploi. Son rôle est de rendre les correspondances plus lisibles : quelles offres sont proches du profil, quelles compétences expliquent le rapprochement, quels écarts subsistent et quelles pistes de montée en compétence peuvent être envisagées.

\section*{Conclusion}
Ce chapitre a établi le socle théorique du mémoire. Le matching emploi--compétences est d'abord un problème d'information imparfaite : les offres, profils, compétences et formations sont décrits avec des vocabulaires hétérogènes. Les systèmes de recommandation, les embeddings, les graphes de connaissances et la \RAG{} apportent chacun une réponse partielle à ce problème, mais aucun ne suffit isolément. La revue conduit donc à une position claire : le système à concevoir doit combiner une mémoire vectorielle pour retrouver rapidement des objets proches, un graphe pour vérifier les relations métier-compétence-formation, et une orchestration agentique pour produire des réponses contrôlées. 
