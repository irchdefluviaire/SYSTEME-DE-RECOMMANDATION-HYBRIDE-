...
\begin{figure}[H]
\centering
\includegraphics[width=0.78\textwidth]{figures/generated/evaluation/hybrid_weights_multi_objective_comparison.png}
...
\section{Interprétation par rapport aux hypothèses}
...
\section{Lecture des sorties graphiques de performance}
...
La troisième sortie graphique, centrée sur les déplacements de rang, explique ce résultat. Neo4j ne se contente pas d'ajouter un score marginal ; il réordonne réellement le pool initial retourné par pgvector. Cette intervention corrige des proximités purement lexicales en donnant plus de poids aux compétences effectivement requises et possédées. Le gain du graphe dépend donc de la qualité des relations et du réglage des poids du score hybride.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.82\textwidth]{figures/generated/evaluation/evaluation_operational_summary.png}
  \caption{Stabilité opérationnelle observée pendant l'évaluation d'ablation}
  \label{fig:eval_operational_summary}
\end{figure}

La figure \ref{fig:eval_operational_summary} complète cette lecture par la stabilité d'exécution. Sur 80 requêtes demandées, 69 ont été évaluées et 11 n'ont pas été retenues dans les métriques finales. Le taux d'écartement atteint donc 13.75\%. Ce résultat n'annule pas les gains mesurés sur Precision@5, Recall@10, NDCG@10 et MRR@10, mais il empêche de conclure à une robustesse de production. Dans l'état actuel, la performance algorithmique du graphe existe, mais elle doit être accompagnée de contrôles d'intégrité et de surveillance d'exécution.

Enfin, aucune figure de calibration du critic n'est interprétée dans ce chapitre, car le dépôt ne contient pas encore de jeu réel \filevar{critic\_calibration\_cases.csv}. Le fichier disponible est seulement un gabarit, avec des lignes fictives. L'utiliser pour produire des courbes RAGAS donnerait des graphiques visibles, mais méthodologiquement faux. La calibration du critic doit donc rester une étape d'évaluation complémentaire, à exécuter uniquement après constitution d'un échantillon réel de questions, réponses, contextes récupérés et labels RAGAS ou humains.
