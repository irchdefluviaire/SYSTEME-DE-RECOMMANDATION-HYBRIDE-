...
L’objectif général de cette étude est de concevoir un système de recommandation hybride emploi-compétences capable de rapprocher des profils de candidats et des offres d’emploi en tenant compte simultanément de la similarité sémantique des textes, des référentiels métiers, compétences et formations, ainsi que des relations explicites entre métiers, compétences, secteurs, niveaux de qualification et domaines de formation. Il ne s’agit donc pas seulement de retrouver des mots communs entre une offre et un profil, mais de construire un dispositif capable d’identifier les écarts de compétences, de classer les correspondances les plus pertinentes et de proposer une trajectoire de montée en compétences interprétable et compatible avec la nomenclature des formations.

Plus spécifiquement, il sera question de :

\begin{itemize}
    \item collecter, nettoyer et normaliser les données relatives aux offres d’emploi, aux profils de candidats et aux référentiels de métiers, de compétences et de formations utilisés dans le projet;
    \item aligner les informations locales avec les référentiels \ESCO{}, \MEPC{} et \NCF{} afin de réduire l’hétérogénéité des intitulés de métiers, de compétences, de niveaux de formation et de domaines de spécialisation;
    \item construire des représentations textuelles exploitables pour le calcul de similarité, notamment les champs de métadonnées structurées et les textes destinés à l’embedding;
    \item adapter un modèle \textit{SentenceTransformer} au domaine emploi-compétences afin d’améliorer la proximité vectorielle entre les descriptions d’offres, les compétences et les profils;
    \item concevoir un graphe de connaissances sous Neo4j représentant les liens entre candidats, offres d’emploi, compétences, métiers, secteurs, employeurs, localisations et référentiels institutionnels;
    \item intégrer la nomenclature de formation dans le graphe afin de relier les niveaux, domaines et spécialités de formation aux métiers visés et aux compétences à développer;
    \item définir une logique de calcul du \emph{skill gap} permettant d’identifier les compétences acquises, les compétences manquantes, les compétences essentielles à prioriser et les formations susceptibles de combler ces écarts;
    \item poser les bases d’un moteur de recommandation hybride combinant similarité sémantique, contraintes de qualification issues de la \NCF{}, relations de graphe et explicabilité des recommandations;
    \item évaluer la pertinence du dispositif à partir de métriques de recherche d’information et d’une analyse qualitative des recommandations produites.
\end{itemize}
...
% ============================================================
\section{Recherche sémantique et Génération Augmentée par Récupération (\RAG{})}
% ============================================================
 
La recherche sémantique et la \RAG{} forment le
pont entre la représentation des profils et des offres d'emploi et la génération de
recommandations personnalisées. Elles permettent de dépasser les limites de la correspondance
lexicale pour identifier des correspondances conceptuelles profondes.
 
% ────────────────────────────────────────────────────────────
\subsection{Recherche par similarité sémantique}
% ────────────────────────────────────────────────────────────
 
%\subsubsection{Similarité cosinus} 
% ────────────────────────────────────────────────────────────
\subsection{Bases de données vectorielles et algorithmes ANN}
% ────────────────────────────────────────────────────────────
...
% ────────────────────────────────────────────────────────────
\subsection{Principe et fonctionnement du RAG native}
% ────────────────────────────────────────────────────────────
La \RAG{} \cite{lewis2020rag} est un paradigme architectural qui combine la puissance de
génération des \LLM{}s avec la précision factuelle de la recherche dans une base de
connaissances externe. Elle répond à deux limitations majeures des \LLM{}s purs :
leur coupure de connaissance temporelle (\textit{knowledge cutoff}) et leur tendance
aux hallucinations.
 


\subsection{\RAG{} et graphes de connaissances}
 
L'intégration du graphe de connaissances dans le pipeline \RAG{} étend le système au-delà de
la recherche vectorielle pure. Lors de la phase de récupération, le graphe permet d'exploiter
des relations structurées : un candidat maîtrisant \textit{TensorFlow} peut se voir suggérer
des offres requérant \textit{PyTorch} si le graphe encode une relation
\og est similaire à \fg{} entre ces frameworks. La propagation d'information à $L$ sauts dans
le graphe révèle des correspondances indirectes invisibles pour la recherche vectorielle pure.
...
% ============================================================
\section{Métriques d'évaluation des systèmes de recommandation}
% ============================================================
 
L'évaluation rigoureuse d'un système de recommandation est indispensable pour valider sa
pertinence et le comparer aux approches existantes. Nous nous concentrons sur les métriques
\textit{offline}, calculées sur des jeux de données annotés.

\subsection{Les differentes fonctions de pertes}
%────────────────────────────────────────────────────────────
\subsection{Métriques de précision du ranking}
% ────────────────────────────────────────────────────────────
...
\begin{table}[H]
  \centering
  \caption{Métriques d'évaluation des systèmes de recommandation emploi-compétences.}
  \label{tab:metrics}
  \small
  \begin{tabularx}{\linewidth}{llXl}
    ...
    \midrule
    \rowcolor{kmeraiteal!12}
    Precision@K & $|\text{pert.} \cap \text{top-}K| / K$ & Proportion de bonnes recommandations & \cite{wasi2024hrgraph,yang2025talentgraph} \\
    \rowcolor{kmeraigreen!8}
    Recall@K & $|\text{pert.} \cap \text{top-}K| / |\text{pert.}|$ & Couverture des pertinents & \cite{wasi2024hrgraph,yang2025talentgraph} \\
    \rowcolor{kmeraiteal!12}
    MRR & $\frac{1}{|Q|}\sum 1/\text{rank}_i$ & Position du 1\textsuperscript{er} résultat pertinent & \cite{du2023lgir} \\
    \rowcolor{kmeraigreen!8}
    NDCG@K & DCG@K / IDCG@K & Qualité du ranking pondérée & Tous \\
    \rowcolor{kmeraiteal!12}
    Cosinus & $\vec{u}\cdot\vec{v}/(\|\vec{u}\|\|\vec{v}\|)$ & Proximité sémantique & \cite{reimers2019sbert} \\
    \rowcolor{kmeraigreen!8}
    MAP@K & Moy. des AP@K & Précision moyenne & \cite{du2023lgir} \\
    \bottomrule
  \end{tabularx}
  \source{Auteur}
\end{table}
...
Le projet s'appuie sur des données locales du marché du travail et sur des référentiels de normalisation. Les sources locales décrivent les offres d'emploi et les demandeurs. Les référentiels \ESCO{}, \MEPC{} et \NCF{} servent à stabiliser les libellés métiers, les compétences et les niveaux de formation.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{p{3.2cm}X p{3.2cm}}
    ...
    \midrule
    Offres d'emploi & Décrire la demande de travail : intitulé, secteur, localisation, niveau d'étude, compétences et contenu textuel de l'offre. & Normalisation, génération du texte d'embedding et construction des paires de fine-tuning. \\
    ...
    \MEPC{} & Fournir une classification nationale des métiers et professions du Cameroun. & Structuration des grands groupes, sous-groupes et groupes de base. \\
    \NCF{} & Fournir une nomenclature nationale des niveaux, domaines et spécialités de formation. & Rattachement des offres et candidats aux niveaux de formation et aux domaines de formation. \\
    \bottomrule
  \end{tabularx}
  \source{Auteur}
\end{table}
...
La préparation des données est organisée autour de quatre étapes principales : la normalisation des offres, la normalisation des candidats, l'alignement des référentiels et la construction des paires d'entraînement. Cette organisation permet de transformer des données initialement hétérogènes en informations exploitables par le modèle sémantique et par le graphe de connaissances.

\subsection{Normalisation des offres}

La normalisation des offres vise à transformer des informations hétérogènes en variables exploitables par le modèle et par le graphe. Le traitement extrait notamment le titre de l'offre, le secteur, la localisation, les niveaux d'étude demandés, les compétences et les détails de l'annonce. Un champ textuel synthétique est construit pour représenter l'offre sous forme de contenu exploitable par un encodeur sémantique.

Une attention particulière est portée au niveau de formation. Les niveaux indiqués dans les offres sont rapprochés de la \NCF{} afin de produire des codes de formation plus homogènes. Cette étape est essentielle, car la recommandation ne doit pas seulement rapprocher des textes : elle doit aussi vérifier que le profil du candidat est compatible avec les exigences de qualification.

\subsection{Normalisation des candidats}

La normalisation des candidats suit la même logique. Les informations disponibles sur le niveau d'étude, le diplôme, l'expérience, les compétences et la mobilité sont harmonisées. Le projet produit un niveau \NCF{} final pour les candidats lorsque l'information disponible le permet. Cette variable sert ensuite à comparer les profils aux exigences des offres et à préparer les relations dans le graphe.
...
Chaque paire associe une requête structurée, composée de métadonnées, à un texte cible composé des compétences et des détails nettoyés de l'offre. Le corpus obtenu comprend 3\,302 paires au total, réparties en 2\,321 paires d'entraînement, 508 paires de validation et 473 paires de test, selon une partition 70/15/15 avec une graine aléatoire fixée à 42.

\section{Fine-tuning du SentenceTransformer}

La composante sémantique du système repose sur le modèle \texttt{all-MiniLM-L6-v2}, un SentenceTransformer produisant des embeddings de dimension 384. Le choix de ce type de modèle est cohérent avec l'objectif de recherche sémantique : rapprocher, dans un espace vectoriel, des offres et des profils qui expriment des besoins ou compétences similaires même lorsque les mots utilisés ne sont pas identiques \cite{reimers2019sbert}.
...
La construction du graphe suit une logique progressive. Elle commence par la définition des types d'entités et des relations, puis intègre les référentiels \ESCO{}, \MEPC{} et \NCF{}, les offres d'emploi, les profils de candidats et les relations complémentaires entre ces objets. Le graphe crée notamment les niveaux de formation, les domaines de formation, les métiers, les compétences et les relations reliant ces entités.
...
\begin{table}[H]
  \centering
  \caption{Fichiers mobilisés pour construire la base de données graphe}
  \label{tab:fichiers_graphe_neo4j}
  \footnotesize
  \begin{tabularx}{\textwidth}{p{3.2cm}p{6.2cm}X}
    \toprule
    \textbf{Bloc du graphe} & \textbf{Fichiers utilisés} & \textbf{Rôle dans la base graphe} \\
    \midrule
    Offres d'emploi & \path{data/raw/offres.xlsx}; \path{data/processed/offres_normalized.parquet} & Création des nœuds \textit{OffreEmploi} et rattachement aux compétences requises, niveaux \NCF{}, secteurs, employeurs et localisations. \\
    Candidats & \path{data/raw/demandeur.xlsx}; \path{data/processed/candidats_normalized.parquet} & Création des nœuds \textit{Candidat} et rattachement aux compétences possédées, niveaux de formation, domaines de formation et préférences professionnelles. \\
    Référentiel \ESCO{} & \path{data/raw/esco/skills_fr.csv}; \path{data/raw/esco/occupations_fr.csv}; \path{data/raw/esco/occupationSkillRelations_fr.csv}; \path{data/raw/esco/skillsHierarchy_fr.csv}; \path{data/raw/esco/ISCOGroups_fr.csv}; \path{data/raw/esco/broaderRelationsSkillPillar_fr.csv} & Construction des nœuds \textit{Compétence}, \textit{Métier}, \textit{GroupeISCO}, \textit{GroupeCompétences} et des relations métier--compétence et hiérarchiques. \\
    Référentiel \MEPC{} & \path{data/raw/MEPC_Nomenclature_Camerounaise_Metiers.xlsx}; \path{data/processed/mepc_grands_groupes.parquet}; \path{data/processed/mepc_sous_groupes.parquet}; \path{data/processed/mepc_groupes_base.parquet} & Structuration des métiers camerounais en grands groupes, sous-groupes et groupes de base, puis alignement avec les métiers \ESCO{}. \\
    Référentiel \NCF{} & \path{data/raw/NCF_Nomenclature_Camerounaise_Formations.xlsx}; \path{data/processed/ncf_niveaux.parquet}; \path{data/processed/ncf_grands_domaines.parquet}; \path{data/processed/ncf_dom_specialises.parquet}; \path{data/processed/ncf_dom_detailles.parquet} & Représentation des niveaux, domaines et spécialités de formation, puis rattachement des candidats, offres et trajectoires de formation. \\
    Alignement métier & \path{data/processed/mapping_isco_mepc_esco.parquet} & Table de correspondance utilisée pour relier les métiers locaux \MEPC{} aux métiers \ESCO{} à partir des codes ISCO/CITP disponibles. \\
    Schéma Neo4j & \path{src/03_knowledge_graph/schema.cypher} & Définition des contraintes d'unicité et des index de recherche utilisés pour stabiliser et accélérer la base graphe. \\
    \bottomrule
  \end{tabularx}
  \source{Auteur}
\end{table}

L'exploitation du graphe couvre plusieurs usages méthodologiques : validation des contraintes relationnelles, calcul des compétences manquantes, recherche d'offres compatibles, scoring graphique, identification des formations associées à un métier cible et analyse de la distribution des niveaux \NCF{}. Le graphe sert donc à la fois de structure d'intégration, de support d'explicabilité et de base pour la recommandation.
