...
\par\vspace{1.5mm}
    {\normalsize\textit{Stage effectué du 10 Mars au 30 juin 2026}}\\[0.8mm]
...
\vspace*{0.4cm}
\begin{center}
\textit{\textbf{Soutenu publiquement le 07 juillet 2026 devant le jury composé de :}}
\end{center}
\vspace*{0.3cm}
\noindent
\begin{minipage}[t]{0.32\textwidth}
\begin{center}
\textbf{Examinateur}\\[0.15cm]
Pr  MELATAGIA Paulin\\[0.1cm]
\textit{Enseignant associé à l'ISSEA}
\end{center}
\end{minipage}%
\begin{minipage}[t]{0.32\textwidth}
\begin{center}
\textbf{Président}\\[0.15cm]
Dr WANSOUWE WANBITCHING\\[0.1cm]
\textit{Enseignant associé à l'ISSEA}
\end{center}
\end{minipage}%
\begin{minipage}[t]{0.32\textwidth}
\begin{center}
\textbf{Encadreur}\\[0.15cm]
Dr FOUTSE Yuehgoh\\[0.1cm]
...
\begin{itemize}
	\item \textbf{Mme. FOUTSE Yuehgoh}, Présidente et Directrice exécutive adjointe de KmerAI, mon encadrante professionnelle, pour son accompagnement, sa confiance et ses orientations tout au long de ce stage ;
	\item \textbf{M. Ulrich Mbou Sob}, Directeur de la Recherche et Développement de KmerAI, pour sa disponibilité, ses remarques pertinentes et son appui dans la conduite du projet ;
	\item \textbf{M. Marcel OPOUMBA}, Directeur Général de l'ISSEA, pour la rigueur académique promue au sein de l'institution et pour la qualité de la formation reçue ;
	\item \textbf{M. Dieudonné KINKIÉLÉLÉ}, Coordonnateur de la filière ISE, pour sa rigueur, son engagement envers notre promotion et l'exemple professionnel qu'il représente ;
...
Le présent mémoire a ainsi été élaboré à l'issue d'un stage professionnel effectué à la Direction de la Recherche et Développement de KmerAI, du 10 mars au 30 juin 2026. Les travaux réalisés dans ce cadre portent sur le thème suivant :

\begin{center}
\begin{minipage}{0.90\textwidth}
\centering
\textbf{\textcolor{darkblue}{CONCEPTION D'UN SYSTÈME DE RECOMMANDATION HYBRIDE POUR LE MATCHING EMPLOI--COMPÉTENCES AU CAMEROUN PAR INTÉGRATION DES LLMs ET DES GRAPHES DE CONNAISSANCES}}
\end{minipage}
\end{center}
\end{spacing}
...
\begin{spacing}{1.5}
Ce mémoire s'inscrit dans le contexte du rapprochement entre offres d'emploi, profils candidats et compétences sur le marché camerounais pour la plateforme KmerAI. La problématique traitée est celle d'un matching souvent limité par les mots-clés, peu explicable et insuffisamment relié aux référentiels métiers et formations. L'objectif principal est donc de concevoir et d'évaluer un système de recommandation hybride capable de combiner recherche sémantique, graphe de connaissances et analyse du \textit{skill gap}.La démarche repose sur un pipeline complet : nettoyage des offres et profils, alignement avec les référentiels \ESCO{}, \MEPC{} et \NCF{}, fine-tuning d'un modèle SentenceTransformer, indexation dans PostgreSQL--pgvector, construction d'un graphe Neo4j et orchestration agentique avec LangGraph. Les données exploitées comprennent 13\,957 offres, 41\,298 candidats, 72\,643 embeddings d'entités et 142 chunks documentaires issus des référentiels. Les résultats montrent que le modèle fine-tuné améliore le retrieval sémantique, avec un Recall@10 de 0.8520 et un NDCG@10 de 0.6336. L'ajout du graphe améliore ensuite le classement du vivier pgvector : le NDCG@10 passe de 0.5575 à 0.7392 et le MRR@10 de 0.6908 à 0.8093. Le critic de génération, calibré à 0,46, sert de garde-fou pour limiter les réponses insuffisamment ancrées dans les preuves récupérées. L'innovation de l'étude réside dans l'articulation entre mémoire vectorielle, mémoire relationnelle Neo4j, score hybride et agent IA orchestré. La principale limite reste l'absence d'une validation humaine indépendante à grande échelle. Il est recommandé d'enrichir progressivement le graphe, de maintenir l'ETL pour intégrer de nouvelles données et de constituer un jeu annoté par des experts métier.

\textbf{Mots-clés :} recommandation emploi-compétences ; système hybride ; graphe de connaissances ; intelligence artificielle générative ; écart de compétences.
\end{spacing}
...
\begin{spacing}{1.5}
This thesis addresses the problem of matching job offers, candidate profiles and skills in the Cameroonian labour market for KmerAI. Keyword-based matching is often insufficient because it provides limited explanations and does not fully exploit occupational and training reference frameworks. The main objective is therefore to design and evaluate a hybrid recommendation system combining semantic retrieval, a knowledge graph and skill gap analysis. The methodology follows an end-to-end pipeline: cleaning job and candidate data, aligning entities with ESCO, MEPC and NCF reference frameworks, fine-tuning a SentenceTransformer model, indexing vectors in PostgreSQL--pgvector, building a Neo4j knowledge graph and orchestrating the workflow with LangGraph. The system uses 13,957 job offers, 41,298 candidate profiles, 72,643 entity embeddings and 142 documentary chunks extracted from reference documents. The results show that the fine-tuned embedding model improves semantic retrieval, reaching a Recall@10 of 0.8520 and an NDCG@10 of 0.6336. Adding the graph improves the ranking of the pgvector candidate pool: NDCG@10 increases from 0.5575 to 0.7392, while MRR@10 increases from 0.6908 to 0.8093. The generation critic, calibrated at 0.46, acts as a safeguard against poorly grounded answers. The innovative aspect of this work lies in the combination of vector memory, Neo4j relational memory, hybrid reranking and agentic orchestration. The main limitation is the lack of large-scale independent human validation. Further work should enrich the graph, maintain the ETL pipeline for new data and build an expert-annotated evaluation set.

\textbf{Keywords:} job-skill recommendation; hybrid system; knowledge graph; generative artificial intelligence; skill gap.
\end{spacing}
...
Le marché du travail camerounais présente un enjeu d’insertion qui ne peut pas être apprécié par le seul taux de chômage. Selon les estimations modélisées de l’OIT diffusées par la Banque mondiale, le chômage des jeunes de 15 à 24 ans s’établit à 6,6\,\% en 2024 et à 6,5\,\% en 2025 \cite{worldbank2026youthcameroon}. Cependant, 23,2\,\% des jeunes n’étaient ni en emploi, ni en études, ni en formation en 2021, dernière observation disponible pour cet indicateur \cite{worldbank2026neetcameroon}. En outre, l’emploi vulnérable représentait 67,8\,\% de l’emploi total en 2024 \cite{worldbank2026vulnerablecameroon}. Ces indicateurs décrivent des dimensions différentes, mais leur lecture conjointe montre qu’un taux de chômage relativement modéré peut coexister avec une insertion fragile et une forte sous-utilisation de la main-d’œuvre, constat également souligné par l’OIT \cite{ilo2024jobsgap}.

Dans ce contexte, l’enjeu ne consiste pas uniquement à créer ou à diffuser des offres, mais aussi à mieux rapprocher les compétences disponibles des besoins exprimés par les employeurs. Les modèles de recherche et d’appariement montrent que des demandeurs d’emploi et des postes vacants peuvent coexister lorsque l’information circule mal et que la rencontre entre profils et emplois demeure imparfaite \cite{mortensen1994job,pissarides2000equilibrium}. À cette friction s’ajoute une asymétrie d’information : les recruteurs observent imparfaitement les compétences réelles des candidats, tandis que ceux-ci connaissent imparfaitement les exigences effectives des postes \cite{akerlof1970lemons,spence1973job}. L’hétérogénéité des intitulés, des descriptions et des référentiels accentue cette difficulté de comparaison.

L’étude se justifie ainsi par le besoin d’un mécanisme d’intermédiation capable de dépasser le filtrage par mots-clés. L’approche retenue combine une recherche sémantique, qui repère les proximités de sens entre profils et offres, et un graphe de connaissances, qui exploite les relations entre candidats, compétences, métiers, formations et emplois. Cette hybridation vise à améliorer le classement des recommandations, à rendre les correspondances explicables et à identifier les compétences manquantes. Elle s’inscrit dans la mission de KmerAI, plateforme camerounaise qui développe et diffuse des usages locaux de l’intelligence artificielle, en proposant une application directement orientée vers l’emploi et la montée en compétences.
\vspace{-0.5cm}
...
\centering
\textit{\textbf{Comment concevoir un système de recommandation hybride, fondé sur les \LLM{} et 
les graphes de connaissances, capable de mesurer les ecarts de competence (\emph{skill gap}) entre un profil 
...
Cette question générale se décline en deux questions de recherche :
\begin{itemize}
    \item \textbf{Q1}~: Comment organiser les offres, les profils, les métiers, les compétences et les formations afin de mieux rapprocher les candidats des emplois disponibles ?
    \item \textbf{Q2}~: L'ajout des liens entre candidats, compétences et offres améliore-t-il la qualité des recommandations par rapport à une recherche fondée uniquement sur les textes ?
...
\vspace{-0.5cm}
\section*{Objectifs visés dans l'étude}
\markright{Objectifs de l'étude}
L'objectif général de cette étude est de \textbf{concevoir et d'évaluer un système de recommandation hybride emploi-compétences} pour appuyer la décision de recrutement et la montée en compétence. Plus spécifiquement, il s'agit de :
\begin{itemize}
    ...
    \item adapter un modèle de type \textit{encodeur} au domaine emploi-compétences et stocker ses embeddings;
    \item construire un graphe des relations candidats-offres-compétences et définir un score hybride de recommandation;
    \item orchestrer et évaluer la pipeline de production des verdicts du système.
\end{itemize}
%\vspace{-0.5cm}
\section*{Intérêt de l'étude }
\markright{Intérêt de l'étude}
L’intérêt de cette étude réside dans une hybridation effective des mécanismes de recommandation. La similarité vectorielle repère les proximités de sens entre profils et offres, tandis que le graphe de connaissances exploite leurs relations avec les compétences, les métiers et les référentiels. La combinaison pondérée de ces deux signaux vise à produire un classement plus pertinent qu’une recherche fondée uniquement sur les mots-clés ou sur la proximité textuelle. Cette méthodologie apporte ainsi aux systèmes de recommandation une lecture à la fois sémantique et relationnelle du matching, tout en permettant d’expliquer les correspondances et d’identifier les compétences manquantes. Sur le plan opérationnel, elle fournit à KmerAI une base adaptable pour appuyer le recrutement et la montée en compétences dans le contexte camerounais.
\vspace{-0.5cm}
\section*{Hypothèses de l'étude}
\markright{Hypothèses de recherche}
Afin de répondre à la problématique, cette étude repose sur trois hypothèses. 
\begin{itemize}
    \item \textbf{H1 :} La mise en ordre des données à l'aide des référentiels métiers et formations, associée à un modèle adapté au vocabulaire de l'emploi, améliore le rapprochement entre profils et offres.
    \item \textbf{H2 :} La prise en compte des liens entre candidats, compétences, métiers et offres améliore le classement obtenu par la recherche sur les textes seule.
    \item \textbf{H3 :} La combinaison de la recherche, du graphe et du contrôle de la réponse produit des recommandations plus pertinentes, plus compréhensibles et capables de signaler les compétences manquantes.
\end{itemize}
\vspace{-0.5cm}
\section*{Méthodologie de l'étude}
\markright{Méthodologie de l'étude}
La démarche adoptée s’inscrit dans un cadre d’ingénierie des données combinant structuration des connaissances et modélisation hybride. Elle repose sur la collecte et la normalisation d’offres d’emploi issues de plusieurs sites web et de profils candidats, puis sur leur alignement avec les référentiels métiers afin d’assurer la cohérence sémantique. Un graphe de connaissances représente les liens entre les entités, tandis qu’une mémoire vectorielle permet de rechercher les contenus sémantiquement proches. Le moteur combine ensuite la similarité vectorielle et le signal relationnel du graphe afin de classer les offres, d’identifier les écarts de compétences et de proposer des trajectoires d’apprentissage. L’évaluation repose notamment sur la précision, le rappel et le NDCG calculés sur les premiers résultats du classement, ainsi que sur la cohérence des réponses générées. Sur le plan technique, Python prend en charge les traitements et l’entraînement, PostgreSQL et Neo4j assurent les mémoires vectorielle et relationnelle, LangGraph orchestre le workflow, tandis que FastAPI et Streamlit exposent le système à l’utilisateur.
\vspace{-0.5cm}
\section*{Plan du travail}
\markright{Plan du travail}
Le mémoire est organisé en deux parties comprenant cinq chapitres. La première partie établit les fondements théoriques et présente l’approche méthodologique retenue, depuis la préparation des données jusqu’à la conception du workflow hybride. La seconde décrit la construction opérationnelle du système, évalue ses performances internes, puis synthétise et discute les résultats au regard de la littérature et des hypothèses de recherche. Une conclusion générale rappelle enfin les principaux enseignements, les limites et les perspectives du travail.
...
Le travail suit une chaîne simple. Les offres, les profils et les référentiels sont d'abord collectés et nettoyés. Les informations comparables, comme les métiers, les compétences, les niveaux de formation et les localisations, sont ensuite harmonisées. Le système recherche alors les offres proches d'un candidat, vérifie les compétences communes et manquantes, classe les résultats, puis produit une explication.
Cette organisation répond à quatre besoins. Premièrement, chaque recommandation doit pouvoir être reliée aux données utilisées. Deuxièmement, le classement ne doit pas dépendre d'un seul indice. Troisièmement, le système doit signaler les compétences déjà acquises et celles qui manquent. Enfin, la réponse rédigée doit rester fidèle aux informations retrouvées \cite{lewis2020rag,es2024ragas}.LangGraph coordonne ces étapes et conserve la trace des outils appelés \cite{langgraphdocs2026}. Si les informations récupérées sont insuffisantes, le système peut effectuer une recherche complémentaire avant de répondre.
...
\begin{figure}[H]
  \centering
  \caption{Vue simplifiée du workflow de recommandation emploi--compétences}
  \label{fig:workflow_methodologique_simplifie}
  \includegraphics[height=0.4\textheight,keepaspectratio]{vrai.png}
...
Les résultats obtenus sont ensuite réunis dans un contexte commun. Pour une recommandation, le système rapproche d'abord le profil des offres, puis vérifie les compétences et les niveaux de formation avant de calculer les écarts. Pour une question documentaire, il récupère les passages utiles et leurs sources. Pour une question relationnelle, il utilise les liens du graphe. Le modèle de langage rédige enfin la réponse à partir de ces éléments. Un module de contrôle, appelé \textit{critic}\footnote{Le \textit{critic} vérifie si la réponse générée est suffisamment appuyée par les informations récupérées. Il ne remplace pas une validation humaine.}, décide si la réponse peut être acceptée ou si une recherche complémentaire est nécessaire. Le workflow se termine par l'affichage de la réponse dans l'API, le chatbot ou l'interface en ligne, avec les traces utiles à sa vérification.
...
\includegraphics[width=0.8\textwidth]{figures/generated/evaluation/embedding_benchmark_plot.png}
\source{Nos travaux, évaluation SentenceTransformer sur le jeu de test offre--profil.}
\end{figure}
...
\includegraphics[width=0.8\textwidth]{figures/generated/evaluation/ablation_metrics_comparison.png}
\source{Nos travaux, ablation du moteur de recommandation sur 69 candidats évalués.}
\end{figure}
...
\includegraphics[width=0.66\textwidth]{figures/generated/evaluation/critic_contrastive_distributions.png}
...
\includegraphics[width=0.66\textwidth]{figures/generated/evaluation/critic_contrastive_threshold_grid.png}
\source{Nos travaux, évaluation du critic à partir des réponses générées et des contextes récupérés.}
\end{figure}
...
Ce mémoire avait pour objectif de concevoir et d'évaluer un système hybride de recommandation emploi--compétences capable de rapprocher profils, offres, compétences et référentiels, tout en expliquant les écarts de compétences. Le travail réalisé montre que cet objectif est atteint sur le plan opérationnel : les données sont normalisées, les représentations sémantiques sont adaptées au domaine, les entités sont indexées dans pgvector, les relations sont structurées dans Neo4j et l'ensemble est orchestré par un workflow agentique.

Les principaux résultats confirment l'intérêt de cette architecture. Le fine-tuning améliore le retrieval sémantique, le graphe améliore le classement du vivier vectoriel et le critic apporte un contrôle d'ancrage de la génération. L'étude montre ainsi qu'une recommandation emploi--compétences ne doit pas se limiter à une proximité textuelle : elle gagne à combiner similarité, relations métier, niveaux de formation, \textit{skill gap} et traçabilité des sources.

Les limites identifiées invitent toutefois à rester prudent. Les résultats reposent sur des métriques internes, un proxy de pertinence et une calibration contrastive du critic. Ils démontrent la cohérence technique du système, mais ne remplacent pas une validation indépendante par des experts métier et des utilisateurs réels. Les données collectées par scraping peuvent également contenir des manques ou des biais de couverture selon les plateformes d'emploi.

Les perspectives prolongent directement ce travail. À court terme, le système devra être intégré au site internet de KmerAI via son API FastAPI et son interface chatbot afin de permettre des tests utilisateurs en conditions réelles. Les artefacts du système, notamment les jeux d'évaluation, les modèles d'embeddings et les sorties GraphRAG, pourront être documentés et soumis à des évaluations reproductibles sur Hugging Face. Le graphe de connaissances devra également être déployé sur Neo4j Aura pour faciliter son exploitation, sa maintenance et son passage à l'échelle.

À moyen terme, il sera nécessaire de constituer un jeu annoté par des recruteurs, conseillers d'orientation et experts formation, puis de mesurer la satisfaction utilisateur, la pertinence métier des recommandations et la qualité des parcours de montée en compétences.
...
\begin{figure}[H]
  ...
  \includegraphics[width=0.62\textwidth]{chatbot.png}
...
\begin{figure}[H]
  ...
  \includegraphics[width=0.5\textwidth]{api1.png}
