...
\titleformat{\section}
  {\normalfont\fontsize{16}{19}\selectfont\bfseries\color{kmeraiteal}}
  {\thesection}{0.8em}{}
\titleformat{\subsection}
  {\normalfont\fontsize{14}{17}\selectfont\bfseries\color{kmeraigreen}}
  {\thesubsection}{0.8em}{}
\titleformat{\subsubsection}
  {\normalfont\fontsize{12}{14}\selectfont\bfseries\color{kmeraired}}
...
\fancyfoot[R]{Mémoire professionnel}
...
\begin{figure}[p]
  ...
  \caption{Architecture méthodologique du workflow agentique de recommandation emploi-compétences}
...
\begin{table}[H]
  ...
  \end{tabularx}
  \tableStyleEnd
  \source{Auteur, à partir de l'architecture du projet}
\end{table}
...
\begin{table}[H]
  ...
  \end{tabularx}
  \tableStyleEnd
  \source{Diagnostic local du système}
\end{table}
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    proc/.style={rectangle, rounded corners=3pt, draw=kmeraiteal, thick, fill=kmeraiteal!8,
      align=center, minimum width=3.2cm, minimum height=0.8cm, font=\small\bfseries},
    critic/.style={diamond, draw=kmeraired, thick, fill=kmeraired!8,
      align=center, aspect=2.0, font=\small\bfseries},
    ...
    \draw[arr] (b) -- (g);
    \draw[arr] (g) -- (c);
    \draw[arr] (c) |- node[pos=0.25, above, font=\scriptsize]{réviser} (x);
    \draw[arr] (x) -| (e);
  \end{tikzpicture}}
  \source{Auteur}
\end{figure}
...
\begin{table}[H]
  ...
  \end{tabularx}
  \tableStyleEnd
  \source{Auteur, à partir du registre d'outils du système}
\end{table}
...
\begin{table}[H]
  ...
  \end{tabularx}
  \tableStyleEnd
  \source{Diagnostic exécuté localement}
\end{table}
...
\section{Objectif et logique générale de l'évaluation}
...
Les résultats présentés dans ce chapitre proviennent d'artefacts exécutables : le benchmark d'embeddings sauvegardé dans \filevar{outputs/evaluation/embedding\_benchmark.json}, l'ablation \filevar{src/07\_evaluation/ablation\_pgvector\_graph.py}, la calibration \filevar{src/07\_evaluation/optimize\_hybrid\_weights.py} et la calibration contrastive du critic \filevar{src/07\_evaluation/calibrate\_critic\_threshold\_contrastive.py}. Les sorties d'ablation sont exportées dans \filevar{outputs/evaluation/pgvector\_vs\_graph}, les sorties Optuna dans \filevar{outputs/evaluation/hybrid\_weight\_optimization}, les sorties du critic dans \filevar{outputs/evaluation/critic\_contrastive\_calibration} et les figures dans \filevar{rapport/figures/generated/evaluation}. Cette traçabilité est importante : les nombres présentés ici ne sont pas des valeurs théoriques, mais des sorties produites par le pipeline local.
...
\section{Évaluation du modèle d'embeddings}
...
\section{Ablation pgvector seul contre pgvector + graphe}
...
\centering
\small
\caption{Synthèse de l'évaluation fonctionnelle du graphe}
\label{tab:eval_functional_graph_summary}
\begin{tabularx}{\textwidth}{XrrX}
\toprule
Test fonctionnel & Volume ou score & Lecture & Interprétation \\
\midrule
Offres avec compétence \filevar{REQUIERT} & 10\,729 / 13\,957 & 76,87\,\% & Couverture suffisante pour le reranking, mais 3\,228 offres restent pénalisées par absence de compétences matérialisées. \\
Candidats avec compétence \filevar{POSSEDE} & 38\,841 / 41\,298 & 94,05\,\% & Le signal candidat est largement disponible pour calculer le taux de match. \\
...
Compatibilité \NCF{} exacte & 13,29\,\% & Même niveau & Le niveau exact est discriminant ; il ne doit pas être remplacé par le seul texte. \\
\bottomrule
\end{tabularx}
\end{table}
...
\section{Analyse du reranking par le graphe}
...
\section{Calibration bayésienne des pondérations du score hybride}

Afin de ne pas conserver des pondérations arbitraires dans le moteur hybride, un module logiciel spécifique a été ajouté : \filevar{src/07\_evaluation/optimize\_hybrid\_weights.py}. Ce module lit les sorties détaillées de l'ablation, optimise les pondérations du score hybride avec Optuna et compare trois fonctions objectif : NDCG@10, moyenne NDCG@10--Recall@10--Precision@10, et moyenne MRR@10--Precision@10. Les sorties sont sauvegardées dans \filevar{outputs/evaluation/hybrid\_weight\_optimization} et les figures correspondantes dans \filevar{rapport/figures/generated/evaluation}.
...
\section{Calibration contrastive du critic de génération}
...
\source{Calibration contrastive du critic, \filevar{critic\_contrastive\_cases.csv}}
\end{figure}
...
\begin{table}[H]
\centering
\caption{Comparaison du seuil courant et du seuil contrastif recommandé}
...
\midrule
0,12 courant & 0,9444 & 6 & 0 & Seuil permissif : il conserve toutes les réponses avec contexte réel, mais accepte encore des contextes mélangés. \\
0,46 recommandé & 0,9901 & 0 & 1 & Seuil prudent : il bloque tous les contextes mélangés, au prix d'une révision inutile. \\
\bottomrule
\end{tabularx}
\tableStyleEnd
\source{Calibration contrastive du critic, \filevar{critic\_contrastive\_threshold\_grid.csv}}
\end{table}

Le résultat le plus important est le suivant : le seuil initial de 0,12 fonctionne comme garde-fou minimal, mais il reste trop permissif dans ce test contrastif. Il accepte 6 contextes mélangés sur 51, soit un taux de fausse acceptation de 11,76\,\%. Le seuil recommandé par le protocole contrastif est 0,46. À ce seuil, aucune réponse associée à un contexte mélangé n'est acceptée, tandis que 50 réponses sur 51 restent acceptées avec leur vrai contexte. La règle retenue est volontairement sévère : minimiser d'abord les fausses acceptations, puis maximiser le F1 des décisions \filevar{accept/revise}.
