...
Le graphe de connaissances organise les entités utiles au raisonnement dans Neo4j : candidats, offres, compétences, métiers, niveaux \NCF{}, groupes \MEPC{}, secteurs, employeurs et localisations.

Son intérêt est de conserver les relations explicites entre ces entités. Par exemple, une offre peut requérir des compétences, un candidat peut posséder certaines compétences, un métier peut être relié à un référentiel, et un document peut définir ou décrire un élément réglementaire. Cette représentation est plus interprétable qu'une simple liste de scores, car elle permet de retracer les liens qui justifient une recommandation \cite{hogan2021knowledge}.

\section{Base vectorielle PostgreSQL et pgvector}

La base vectorielle stocke les embeddings dans PostgreSQL avec l'extension pgvector. Les entités du projet sont encodées avec le modèle d'embeddings, puis leurs vecteurs sont indexés pour permettre une recherche rapide par similarité sémantique.

Son rôle est complémentaire à celui du graphe. pgvector retrouve rapidement les éléments proches dans l'espace des embeddings ; Neo4j vérifie ensuite les relations, les contraintes et les chemins explicables. La recommandation ne repose donc pas uniquement sur la proximité textuelle : elle peut être enrichie par la structure métier du graphe.

\section{Ingestion des PDF réglementaires}

L'ingestion documentaire concerne les documents officiels utilisés dans le projet, notamment les PDF liés à la \NCF{}, à la \MEPC{} et aux diplômes. Ces documents sont découpés en chunks, transformés en embeddings, indexés dans pgvector et reliés au graphe lorsque les informations correspondantes sont détectées.

Cette étape donne une base documentaire au système. Au lieu de produire seulement une recommandation, le système peut s'appuyer sur des passages issus des référentiels pour expliquer certains niveaux de formation, métiers ou domaines. Cela renforce l'explicabilité, surtout dans un contexte où les nomenclatures officielles doivent rester traçables.

\section{Moteur de recommandation hybride}

Le moteur GraphRAG combine les informations issues de la base vectorielle et du graphe. Il charge un candidat, construit un contexte à partir de pgvector et de Neo4j, puis produit des recommandations, un \emph{skill gap} et une roadmap.

Le caractère hybride vient du fait que plusieurs signaux sont combinés : la similarité vectorielle, les relations du graphe, les compétences possédées et requises, les contraintes de niveau ou de secteur, et les éléments documentaires disponibles. Le résultat attendu n'est donc pas seulement une liste d'offres, mais une recommandation accompagnée d'une explication lisible \cite{burke2002hybrid,ricci2011recommender}.

\section{Orchestration, API et interfaces}

L'orchestration et les interfaces rendent le système exploitable. Le projet contient une API FastAPI, un workflow agentique LangGraph, une interface Streamlit et une CLI interactive. Ces éléments servent à interroger le système, afficher les traces du raisonnement et rendre les recommandations accessibles à un utilisateur.

Cette couche ne remplace pas le moteur de recommandation ; elle l'expose. Elle permet de passer d'un traitement technique exécuté en scripts à une interaction plus opérationnelle : recherche d'offres, analyse de profil, conversation avec le chatbot, ou consultation des étapes du workflow.

\section{Sorties et évaluation}

Les sorties et l'évaluation permettent de contrôler ce que produit réellement le système. Le projet prévoit des recommandations d'offres, du matching candidat--emploi, une analyse du \emph{skill gap}, une roadmap de formation, ainsi que des rapports et fichiers d'évaluation. L'évaluation porte notamment sur les embeddings, le GraphRAG, les scores et la latence.

Cette étape est nécessaire pour ne pas juger le système uniquement sur son architecture. Une méthodologie sérieuse doit observer les sorties : quelles offres sont recommandées, pourquoi elles le sont, quelles compétences manquent, quelles sources sont mobilisées et quelles limites apparaissent dans les métriques. C'est ce contrôle qui permet de passer d'une démonstration technique à une analyse défendable dans le mémoire.
