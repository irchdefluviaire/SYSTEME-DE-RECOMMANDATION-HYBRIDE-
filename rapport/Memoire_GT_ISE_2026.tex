...
%  \label{fig:approche_methodologique}
 % \includegraphics[width=0.75\textwidth]{figures/approche.png}
%  \source{Auteur}
%\end{figure}
\section{Sources et traitements de données mobilisées}
Les sources de données constituent le point de départ de la solution. 
...
\subsection{Pipeline de préparation des données de l'étude}
\begin{table}[H]
  \centering
  ...
  \begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash\bfseries}p{2.7cm}
    ...
    \tabheader{Donnée} &
    \tabheader{Source} &
    \tabheader{Rôle méthodologique} &
    \tabheader{Utilisation dans le projet} \\
    \midrule
    Offres d'emploi &
    Collecte KmerAI sur sites d'annonces &
    Décrire la demande de travail : intitulé, secteur, localisation, employeur, niveau d'étude, compétences et contenu textuel de l'annonce. &
    Normalisation, génération de \filevar{text\_to\_embed}, construction des paires de fine-tuning et création des nœuds d'offres. \\
    Demandeurs d'emploi &
    Base locale de profils candidats &
    Décrire l'offre de travail : profil, diplôme, niveau d'étude, expérience, compétences et mobilité. &
    Normalisation des profils, production de \filevar{text\_to\_embed} et rattachement aux niveaux de formation. \\
    \ESCO{} &
    Référentiel européen métiers--compétences &
    Fournir une nomenclature internationale de métiers, compétences et relations métier--compétence. &
    Chargement des occupations, compétences, groupes ISCO et relations occupation--compétence. \\
    \MEPC{} &
    INS Cameroun, nomenclature des métiers 2013 &
    Fournir une classification nationale des métiers, emplois et professions du Cameroun. &
    Structuration en grands groupes, sous-groupes et groupes de base, puis alignement avec les métiers \ESCO{}. \\
    \NCF{} &
    INS Cameroun, nomenclature des formations 2017 &
    Fournir une nomenclature nationale des niveaux, domaines et spécialités de formation. &
    Structuration en niveaux, grands domaines, domaines spécialisés et domaines détaillés, puis rattachement aux profils et aux offres. \\
    \bottomrule
  \end{tabularx}
  \tableStyleEnd
  \source{Auteur}
...
\subsubsection{Normalisation des offres d'emplois}
...
\subsubsection{Normalisation des données sur les profils des demandeurs d'emploi}
...
\subsection{Alignement des référentiels métiers et compétences}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{p{3.4cm}p{4.1cm}X}
    \rowcolor{darkblue}
    \tabheader{Table finale} & \tabheader{Unité statistique} & \tabheader{Champs structurants} \\
    \midrule
    \filevar{mepc\_grands\_groupes} & Grand groupe de métiers & \filevar{code}, \filevar{intitule}, \filevar{notes\_explicatives}, \filevar{codes\_citp}, \filevar{text\_to\_embed}. \\
    \filevar{mepc\_sous\_groupes} & Sous-groupe rattaché à un grand groupe & \filevar{code}, \filevar{intitule}, \filevar{notes\_explicatives}, \filevar{codes\_citp}, \filevar{code\_grand\_groupe}, \filevar{text\_to\_embed}. \\
    \filevar{mepc\_groupes\_base} & Groupe de base, niveau le plus fin retenu pour le matching métier & \filevar{code}, \filevar{intitule}, \filevar{notes\_explicatives}, \filevar{codes\_citp}, \filevar{code\_sous\_groupe}, \filevar{code\_grand\_groupe}, \filevar{text\_to\_embed}. \\
    \bottomrule
  \end{tabularx}
  \tableStyleEnd
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{p{3.5cm}p{4cm}X}
    ...
    \tabheader{Table finale} & \tabheader{Unité statistique} & \tabheader{Champs structurants} \\
    \midrule
    \filevar{ncf\_niveaux} & Niveau de formation & \filevar{code}, \filevar{intitule}, \filevar{explication}, \filevar{text\_to\_embed}. \\
    \filevar{ncf\_grands\_domaines} & Grand domaine de formation & \filevar{code}, \filevar{intitule}, \filevar{explication}, \filevar{text\_to\_embed}. \\
    \filevar{ncf\_dom\_specialises} & Domaine spécialisé rattaché à un grand domaine & \filevar{code}, \filevar{intitule}, \filevar{explication}, \filevar{code\_grand\_domaine}, \filevar{text\_to\_embed}. \\
    \filevar{ncf\_dom\_detailles} & Domaine détaillé, niveau le plus fin retenu pour rattacher les formations & \filevar{code}, \filevar{intitule}, \filevar{explication}, \filevar{code\_dom\_specialise}, \filevar{code\_grand\_domaine}, \filevar{text\_to\_embed}. \\
    \bottomrule
  \end{tabularx}
  \tableStyleEnd
...
%\subsection{ETL et normalisation}

%L'ETL assure le passage des données brutes vers des données exploitables. Son rôle est de transformer des sources hétérogènes en tables propres et cohérentes pour les étapes suivantes. Le pipeline normalise les offres, normalise les candidats, aligne les référentiels \MEPC{}, \NCF{} et \ESCO{}, puis construit les paires utilisées pour le fine-tuning.

%Concrètement, cette étape nettoie les textes, harmonise les libellés, structure les champs importants, extrait ou organise les compétences et rapproche les niveaux de formation. C'est la base technique du mémoire : si cette étape est faible, tout le reste devient fragile, car le modèle et le graphe travailleront sur des données mal stabilisées.

\section{Fine-tuning du Sentence Transformer}
...
%\subsection{Fine-tuning du SentenceTransformer}
...
%    \rowcolor{darkblue}
%    \tabheader{Paramètre} & \tabheader{Valeur utilisée} \\
%    \midrule
%    Modèle de base & \filevar{all-MiniLM-L6-v2} \\
%    Dimension des embeddings & 384 \\
%    Fonction de perte & \filevar{MultipleNegativesRankingLoss} \\
%    Nombre d'époques & 5 \\
%    Taille de batch & 32 \\
...
%    Temps d'entraînement & 6\,630,9 secondes \\
 ...
 %   Critère de sélection & \filevar{val-information-retrieval\_cosine\_ndcg@10} \\
  %  \bottomrule
 % \end{tabularx}
 % \tableStyleEnd
  %\source{Auteur}
...
\subsection{Validation et choix du modèle d'embeddings}

Le choix du modèle d'embeddings ne peut pas reposer sur une impression qualitative tirée de quelques requêtes. Dans ce projet, l'embedding est le premier filtre du système : il transforme une requête courte, issue des métadonnées d'une offre ou d'un candidat, en une liste top-$K$ de textes candidats à examiner ensuite par le graphe, les règles de compétences et le module \RAG{}. Le modèle est donc validé comme un \textit{retriever} sémantique, conformément à l'usage des SentenceTransformers pour la recherche par similarité \cite{reimers2019sbert} et aux pratiques de benchmark de recherche d'information \cite{thakur2021beir,muennighoff2022mteb}.

Formellement, soit une requête $q_i$ et un ensemble de documents candidats $\mathcal{D}=\{d_1,\ldots,d_n\}$. Le modèle encode la requête et chaque document dans un même espace vectoriel :
\[
\vec{z}_{q_i}=f_\theta(q_i), \qquad \vec{z}_{d_j}=f_\theta(d_j).
\]
Le score de classement est la similarité cosinus :
\[
s(q_i,d_j)=\cos(\vec{z}_{q_i},\vec{z}_{d_j})
=\frac{\vec{z}_{q_i}^{\top}\vec{z}_{d_j}}{\|\vec{z}_{q_i}\|\,\|\vec{z}_{d_j}\|}.
\]
...
Les métriques retenues mesurent ce comportement de classement. Le \textit{Recall@K} indique si le document pertinent est présent dans les $K$ premiers résultats :
\[
\operatorname{Recall@K}(q_i)=\frac{|\mathcal{R}_i \cap \mathcal{T}_{i,K}|}{|\mathcal{R}_i|},
\]
où $\mathcal{R}_i$ désigne l'ensemble des documents pertinents et $\mathcal{T}_{i,K}$ les $K$ premiers documents retournés. Lorsque chaque requête possède un seul document positif, cette quantité vaut 1 si le bon document est dans le top-$K$, et 0 sinon. Le \textit{MRR@K} pénalise davantage un document pertinent placé trop bas :
\[
\operatorname{MRR@K}=\frac{1}{Q}\sum_{i=1}^{Q}\frac{1}{\operatorname{rang}_i},
\]
avec $\operatorname{rang}_i$ le rang du premier document pertinent, à condition qu'il apparaisse dans le top-$K$. Le \textit{NDCG@K} évalue la qualité globale du classement en appliquant une décote logarithmique aux positions tardives :
\[
\operatorname{DCG@K}=\sum_{r=1}^{K}\frac{2^{rel_r}-1}{\log_2(r+1)}, \qquad
\operatorname{NDCG@K}=\frac{\operatorname{DCG@K}}{\operatorname{IDCG@K}},
\]
où $rel_r$ est la pertinence du document placé au rang $r$ et $\operatorname{IDCG@K}$ le score obtenu par le classement idéal. Enfin, la \textit{MAP@K} synthétise la précision moyenne observée aux rangs où des documents pertinents apparaissent.

Un exemple simple montre pourquoi ces métriques sont plus informatives qu'une précision globale. Supposons une requête correspondant à une offre de \og développeur Python junior \fg{} et cinq documents retournés : $[d_3,d_1,d_8,d_5,d_2]$. Si le document attendu est $d_1$, alors $\operatorname{Recall@1}=0$, $\operatorname{Recall@5}=1$ et $\operatorname{MRR@5}=1/2$. Le modèle n'a pas échoué à retrouver l'information, mais il l'a placée au deuxième rang. Dans un moteur de recommandation, cette nuance est essentielle : un bon générateur de shortlist peut être utile même s'il ne classe pas toujours le meilleur candidat en première position.

\section{Construction du graphe de connaissances avec Neo4j}
...
Dans l'état actuel du projet, le graphe Neo4j contient \textbf{43\,008 nœuds} répartis sur \textbf{18 types de labels}. Le tableau~\ref{tab:volume_neo4j} détaille les volumes par famille d'entités.

\begin{table}[H]
  \centering
  \caption{Volume de nœuds dans le graphe Neo4j}
  \label{tab:volume_neo4j}
  \tableStyleStart
  \begin{tabularx}{\textwidth}{p{5.5cm}rX}
    \rowcolor{darkblue}
    \tabheader{Label(s) de nœud} & \tabheader{Volume} & \tabheader{Bloc} \\
    \midrule
    \filevar{OffreEmploi} & 15\,722 & Données d'emploi \\
    \filevar{Compétence} & 13\,939 & Compétences \ESCO{} \\
    \filevar{Métier} & 3\,039 & Métiers \ESCO{} \\
    \filevar{Candidat} & 1\,105 & Données d'emploi \\
    \filevar{GroupeBaseMEPC} & 209 & Référentiel \MEPC{} \\
    \filevar{DomaineDétailléNCF} & 201 & Référentiel \NCF{} \\
    Autres nœuds référentiels, contextuels et documentaires & $\sim$8\,793 & NCF, MEPC, ISCO, Secteur, Localisation, Employeur, DocChunk… \\
    \midrule
    \textbf{Total} & \textbf{43\,008} & 18 types de labels \\
    \bottomrule
  \end{tabularx}
  \tableStyleEnd
  \source{Auteur, extrait via \texttt{MATCH (n) RETURN labels(n), count(n)}}
\end{table}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{p{4cm}X}
    \rowcolor{darkblue}
    \tabheader{Famille d'entités} & \tabheader{Exemples représentés} \\
    \midrule
...
% -------------------------------------------------------
% §2.7  Base vectorielle pgvector
% -------------------------------------------------------
\section{Base vectorielle PostgreSQL et pgvector}
\label{sec:pgvector}

La base vectorielle constitue le second composant de stockage du système. Elle repose sur PostgreSQL~16 et l'extension \texttt{pgvector}, qui ajoute un type de colonne \texttt{vector(384)} et des opérateurs de similarité cosinus, permettant des recherches approximatives par plus proches voisins directement dans la base relationnelle. Ce choix maintient la cohérence avec l'infrastructure PostgreSQL existante et évite d'introduire une base vectorielle tierce.

\subsection{Schéma de la table d'embeddings}

Toutes les entités du projet partagent une table unique \texttt{entity\_embeddings}. Cette centralisation simplifie les requêtes multi-types et l'accès uniforme depuis le workflow général :

\begin{table}[H]
  \centering
  \caption{Structure de la table \filevar{entity\_embeddings} dans pgvector}
  \label{tab:structure_pgvector}
  \tableStyleStart
  \begin{tabularx}{\textwidth}{p{3.2cm}p{2.8cm}X}
    \rowcolor{darkblue}
    \tabheader{Colonne} & \tabheader{Type SQL} & \tabheader{Rôle} \\
    \midrule
    \filevar{entity\_id} & \filevar{VARCHAR} & Identifiant de l'entité source (\filevar{offre\_id}, \filevar{candidat\_id}, URI \ESCO{}, code NCF…). \\
    \filevar{entity\_type} & \filevar{VARCHAR} & Type sémantique : \filevar{OFFRE\_EMPLOI}, \filevar{CANDIDAT}, \filevar{COMPETENCE}, \filevar{METIER}, \filevar{DOMAINE\_DETAILLE\_NCF}, \filevar{GROUPE\_BASE\_MEPC}. \\
    \filevar{source} & \filevar{VARCHAR} & Origine de la donnée (KmerAI, ESCO, INS-NCF, INS-MEPC). \\
    \filevar{label} & \filevar{TEXT} & Libellé court utilisé dans les réponses du système. \\
    \filevar{text\_origin} & \filevar{TEXT} & Texte brut ayant servi à produire l'embedding (\filevar{text\_to\_embed}). \\
    \filevar{embedding} & \filevar{vector(384)} & Vecteur dense de dimension 384, produit par le SentenceTransformer fine-tuné. \\
    \filevar{created\_at} & \filevar{TIMESTAMP} & Horodatage d'indexation. \\
    \bottomrule
  \end{tabularx}
  \tableStyleEnd
  \source{Auteur}
...
\subsection{Indexation HNSW et requête de similarité}

Pour garantir des temps de réponse compatibles avec un usage interactif, un index HNSW (\textit{Hierarchical Navigable Small World}) est construit sur la colonne \texttt{embedding}, avec $m = 16$ connexions par nœud et $ef\_construction = 64$. La recherche utilise l'opérateur de distance cosinus \texttt{<=>} de pgvector. La requête top-$K$ se formule :

\begin{equation}
\text{top-}K = \underset{d \in \mathcal{D}}{\text{argmin}^{K}}\; \bigl(1 - \cos(\mathbf{q}, \mathbf{d})\bigr)
\label{eq:topk_pgvector}
\end{equation}
...
% -------------------------------------------------------
% §2.8  Ingestion des PDF réglementaires
% -------------------------------------------------------
\section{Ingestion des PDF réglementaires}
\label{sec:pdf_ingestion}

Outre les données structurées, le système intègre le contenu textuel des référentiels officiels sous forme de fragments documentaires (\textit{DocChunks}). Cette couche documentaire permet au moteur agentique de citer des passages précis des nomenclatures \NCF{} et \MEPC{} en réponse à une question portant sur les formations, les métiers ou les niveaux de qualification.

\subsection{Stratégie de découpage et identifiant de chunk}

Les documents PDF (\NCF{} 2017, \MEPC{} 2013) sont découpés en segments de longueur contrôlée. Chaque segment est identifié par un \texttt{chunk\_id} structuré selon le schéma :

\begin{equation}
\texttt{chunk\_id} = \texttt{<SOURCE>\_<ANNEE>\_p<PAGE>\_c<INDEX>}
\label{eq:chunk_id}
\end{equation}

Par exemple, \texttt{NCF\_2017\_p008\_c0001} désigne le premier chunk de la page~8 du document \NCF{} 2017. Ce schéma d'identifiant permet de reconstruire la localisation exacte du passage dans le document source et de le citer de façon traçable dans les réponses générées.

\subsection{Stockage hybride et liaison au graphe}

Chaque chunk est stocké dans la table \texttt{doc\_chunks} avec son texte, son identifiant et ses métadonnées de page. Il reçoit un embedding vectoriel indexé dans \texttt{entity\_embeddings} pour la recherche sémantique. En parallèle, il est représenté comme un nœud \texttt{DocChunk} dans Neo4j, relié au nœud \texttt{DocumentReferentiel} parent par la relation fonctionnelle \texttt{EXTRAIT\_DE} :

\begin{table}[H]
  \centering
  \caption{Résumé de l'ingestion documentaire}
  \label{tab:ingestion_pdf}
  \tableStyleStart
  \begin{tabularx}{\textwidth}{p{4cm}p{2.8cm}X}
    \rowcolor{darkblue}
    \tabheader{Document} & \tabheader{Chunks indexés} & \tabheader{Relation Neo4j} \\
    \midrule
    \NCF{} 2017 (INS Cameroun) & 142 & \filevar{(:DocChunk)-[:EXTRAIT\_DE]->(:DocumentReferentiel)} \\
    \MEPC{} 2013 (INS Cameroun) & En cours & \filevar{(:DocChunk)-[:EXTRAIT\_DE]->(:DocumentReferentiel)} \\
    \bottomrule
  \end{tabularx}
  \tableStyleEnd
  \source{Auteur}
...
Lors de la génération d'une réponse, le \LLM{} cite le chunk mobilisé sous la forme \texttt{[Source: NCF\_2017, chunk: NCF\_2017\_p008\_c0001, p.8]}, assurant la traçabilité documentaire de chaque élément produit par le système.

% -------------------------------------------------------
% §2.9  Moteur de recommandation hybride
% -------------------------------------------------------
\section{Moteur de recommandation hybride}
\label{sec:moteur_hybride}
...
\subsection{Architecture de récupération à deux passes}

La récupération s'effectue en deux passes parallèles. La première passe interroge \texttt{pgvector} par similarité cosinus pour identifier les $K$ entités les plus proches de la requête dans l'espace sémantique. La seconde passe interroge Neo4j par des requêtes Cypher pour extraire les relations de compétences, les niveaux \NCF{} et les alignements \MEPC{}.

Les résultats des deux passes sont fusionnés dans une fenêtre de contexte structurée transmise au \LLM{} pour la génération de la réponse. La recommandation n'est donc pas une simple liste de scores : c'est une synthèse argumentée appuyée sur des faits vérifiables dans les deux bases, conformément aux principes de la \RAG{} appliquée aux graphes \cite{lewis2020rag,hogan2021knowledge}.

\subsection{Score de compatibilité composite}

Pour le classement des candidats en mode \textit{matching}, un score composite est calculé :

\begin{equation}
S_{hyb}(c, o) = \alpha\, S_{sem}(c, o)
              + \beta\, S_{graph}(c, o)
              + \gamma\, C_{ncf}(c, o)
              + \delta\, A_{sect}(c, o)
\label{eq:score_hybride}
\end{equation}
...
\begin{table}[H]
  \centering
  \caption{Composantes du score hybride de recommandation $S_{hyb}$}
  \label{tab:composantes_score_hybride}
  \tableStyleStart
  \begin{tabularx}{\textwidth}{p{1.0cm}p{3.5cm}lX}
    \rowcolor{darkblue}
    \tabheader{Terme} & \tabheader{Composante} & \tabheader{Poids} & \tabheader{Définition et calcul} \\
    \midrule
...
La prédominance de $S_{sem}$ ($\alpha = 0{,}5$) reflète la fiabilité du SentenceTransformer fine-tuné comme premier filtre. Les composantes $C_{ncf}$ et $A_{sect}$ apportent les contraintes institutionnelles qui ne sont pas capturables par la seule proximité vectorielle \cite{burke2002hybrid,ricci2011recommender}.

\subsection{Pipeline Text2Cypher pour l'interrogation du graphe}
...
\begin{enumerate}
  \item \textbf{Modèle GGUF local} (\texttt{llama3.1-8b-text2cypher-Q4\_K\_M.gguf}) : traduit directement la question en requête Cypher sans dépendance réseau.
  \item \textbf{LLM via OpenRouter} : si le modèle local est absent ou échoue, la question est transmise à un \LLM{} externe pour la génération de la requête Cypher.
  \item \textbf{Templates paramétrés} : en dernier recours, sept intentions prédéfinies (\filevar{competence\_lookup}, \filevar{referentiel\_lookup}, \filevar{candidat\_lookup}, \filevar{offre\_lookup}, \filevar{metier\_lookup}, \filevar{ncf\_metier\_lookup}, \filevar{default}) produisent des requêtes Cypher éprouvées utilisant uniquement les relations fonctionnelles du graphe.
\end{enumerate}
...
% -------------------------------------------------------
% §2.10  Workflow agentique LangGraph
% -------------------------------------------------------
\section{Workflow agentique LangGraph pour la recommandation}
\label{sec:langgraph}
...
\subsection{Architecture en cinq nœuds}

Le graphe de traitement comprend cinq nœuds exécutés séquentiellement, chaque nœud enrichissant un état partagé :

\begin{figure}[H]
  \centering
  \caption{Graphe d'exécution du workflow agentique LangGraph}
  \label{fig:langgraph_workflow}
  \resizebox{0.85\textwidth}{!}{%
  \begin{tikzpicture}[
    node distance=0.80cm and 3.5cm,
    wnode/.style={rectangle, rounded corners=4pt, draw=kmeraiteal, very thick,
                  fill=kmeraiteal!10, align=center, minimum width=4.0cm,
                  minimum height=0.88cm, font=\small\bfseries},
    tool/.style={rectangle, rounded corners=2pt, draw=kmeraiorange, thick,
                 fill=kmeraiorange!10, align=center, minimum width=3.0cm,
                 minimum height=0.68cm, font=\scriptsize},
    arr/.style={-{Stealth[length=2.5mm]}, thick, draw=kmeraigray},
    darr/.style={-{Stealth[length=2mm]}, thick, dashed, draw=kmeraiorange!80!black},
    lab/.style={font=\scriptsize, fill=white, inner sep=1.5pt}
  ]
    \node[wnode] (n1) {1.\ \texttt{analyse\_request}};
    \node[wnode, below=of n1] (n2) {2.\ \texttt{plan\_tools}};
    \node[wnode, below=of n2] (n3) {3.\ \texttt{execute\_tools}};
    \node[wnode, below=of n3] (n4) {4.\ \texttt{build\_context}};
    \node[wnode, below=of n4] (n5) {5.\ \texttt{generate\_final\_answer}};

    \node[tool, right=3.0cm of n3, yshift= 0.75cm] (t2) {Neo4j\\(Text2Cypher)};
    \node[tool, right=3.0cm of n3]                 (t1) {pgvector\\(HNSW top-$K$)};
    \node[tool, right=3.0cm of n3, yshift=-0.75cm] (t3) {DocChunks\\(référentiels PDF)};
    ...
    \draw[darr] (n3.east) to[out=0,in=180] (t2.west);
    \draw[darr] (n3.east)                -- (t1.west);
    \draw[darr] (n3.east) to[out=0,in=180] (t3.west);
  \end{tikzpicture}}
  \source{Auteur}
\end{figure}

Le nœud \texttt{analyse\_request} détecte l'intention de l'utilisateur et extrait les entités pertinentes (identifiant candidat, type de use-case, paramètres). Le nœud \texttt{plan\_tools} sélectionne les outils à appeler selon le use-case. Le nœud \texttt{execute\_tools} appelle pgvector, Neo4j et les DocChunks selon le plan. Le nœud \texttt{build\_context} agrège et formate les résultats dans une fenêtre de contexte structurée. Le nœud \texttt{generate\_final\_answer} génère la réponse en langage naturel via le \LLM{}, en citant les sources mobilisées.

\subsection{Routage par use-case du systeme de recommandation}

À l'étape \texttt{analyse\_request}, le système identifie l'intention parmi sept use-cases, qui déterminent quels outils seront activés :

\begin{table}[H]
  \centering
  \caption{Use-cases supportés par le workflow agentique}
  \label{tab:usecases_langgraph}
  \tableStyleStart
  \begin{tabularx}{\textwidth}{p{4.0cm}X}
    \rowcolor{darkblue}
    \tabheader{Use-case} & \tabheader{Description et outils activés} \\
    \midrule
    \filevar{diagnostic} & Analyse du profil candidat : compétences, niveau \NCF{}, secteur, lacunes. Outils : Neo4j + pgvector. \\
    \filevar{recommendation\_candidat} & Recommandation d'offres compatibles. Outils : pgvector (top-$K$), Neo4j (contraintes), score $S_{hyb}$. \\
    \filevar{skill\_gap\_roadmap} & Compétences manquantes et roadmap de formation. Outils : Neo4j (compétences), pgvector (formations proches). \\
    \filevar{referentiel} & Questions sur les nomenclatures \NCF{} et \MEPC{}. Outils : DocChunks + Neo4j. \\
    \filevar{graph\_query} & Requête libre sur le graphe via Text2Cypher. Outil : Neo4j (3 niveaux). \\
    \filevar{orientation\_metier} & Métiers et trajectoires compatibles avec un profil. Outils : pgvector (métiers), Neo4j (domaines \NCF{}). \\
    \filevar{recherche\_generale} & Questions générales sur le marché du travail. Outils : pgvector + DocChunks si pertinent. \\
    \bottomrule
  \end{tabularx}
  \tableStyleEnd
  \source{Auteur}
...
\subsection{Intégration LLM et traçabilité des sources}

Le nœud \texttt{generate\_final\_answer} appelle un \LLM{} via l'API OpenRouter. La configuration retient \texttt{openai/gpt-oss-20b:free} comme modèle principal, avec une chaîne de secours de quatre modèles alternatifs gratuits : \texttt{meta-llama/llama-3.3-70b-instruct:free}, \texttt{deepseek/deepseek-v4-flash:free}, \texttt{google/gemma-4-31b-it:free} et \texttt{meta-llama/llama-3.2-3b-instruct:free}.

Le \LLM{} est instruit de citer explicitement chaque information mobilisée : \texttt{[Compétence: "libellé", uri: esco:...]} pour les compétences \ESCO{}, \texttt{[Source: NCF\_2017, chunk: NCF\_2017\_p008\_c0001, p.8]} pour les passages documentaires, et \texttt{[pgvector: "libellé", id: offre\_id]} pour les offres retrouvées par similarité. Cette instrumentation garantit la traçabilité de chaque élément de la réponse jusqu'à sa source dans la base de données.

% -------------------------------------------------------
% §2.11  API, orchestration et interfaces
% -------------------------------------------------------
\section{API, orchestration et interfaces}
\label{sec:api_interfaces}
...
Ces composants ne modifient pas la logique de recommandation : ils l'exposent à des utilisateurs finaux ou à des systèmes tiers, rendant le système opérationnel au-delà du prototypage.

\section{Évaluation de la génération du système}

L'évaluation d'un système \RAG{} ne peut pas se réduire à un \og vibe check \fg{}, c'est-à-dire à lire quelques réponses et à les trouver convaincantes. Cette pratique est insuffisante pour un système destiné à orienter des demandeurs d'emploi, des formations ou des offres : une réponse fluide peut être fausse, une réponse correcte peut être mal sourcée, et une réponse pertinente peut cacher une récupération documentaire médiocre. L'évaluation doit donc séparer deux étages : la récupération des preuves et la génération de la réponse. Cette séparation reprend l'intuition centrale du \RAG{} : le modèle génératif ne doit pas seulement produire un texte plausible, il doit produire un texte contraint par des sources retrouvées \cite{lewis2020rag,es2024ragas}.

\subsection{Évaluation de la récupération documentaire}
...
\[
\operatorname{Precision@K}(q_i)=\frac{|\mathcal{R}_i \cap \mathcal{T}_{i,K}|}{K}.
\]
Le rappel top-$K$ mesure la couverture des preuves attendues :
\[
\operatorname{Recall@K}(q_i)=\frac{|\mathcal{R}_i \cap \mathcal{T}_{i,K}|}{|\mathcal{R}_i|}.
\]
Le \textit{Hit Rate@K} indique si au moins une preuve pertinente apparaît dans les premiers résultats :
\[
\operatorname{HitRate@K}(q_i)=\mathbb{1}\{|\mathcal{R}_i \cap \mathcal{T}_{i,K}|>0\}.
\]
Le \textit{MRR@K} récompense les systèmes qui placent la première preuve utile très haut :
\[
\operatorname{MRR@K}=\frac{1}{Q}\sum_{i=1}^{Q}\frac{1}{\operatorname{rang}_i}.
\]
Le \textit{NDCG@K} complète ces métriques lorsque plusieurs preuves ont des degrés de pertinence différents :
\[
\operatorname{NDCG@K}=\frac{1}{\operatorname{IDCG@K}}\sum_{r=1}^{K}\frac{2^{rel_r}-1}{\log_2(r+1)}.
\]
...
\subsection{Évaluation de la réponse générée}

La deuxième question est : une fois les preuves récupérées, le système produit-il une réponse fiable ? Trois critères sont utilisés. La \textit{fidélité} vérifie que la réponse ne contient pas d'information absente du contexte récupéré. Si $A_i$ est la réponse générée et $C_i$ le contexte fourni au \LLM{}, on peut représenter la fidélité comme une fonction de couverture des assertions :
\[
\operatorname{Faithfulness}(A_i,C_i)=\frac{\#\{\text{assertions de } A_i \text{ supportées par } C_i\}}{\#\{\text{assertions vérifiables de } A_i\}}.
\]
...
La \textit{pertinence de la réponse} mesure si le texte répond directement à la question :
\[
\operatorname{Rel}(A_i,q_i)=g(q_i,A_i),
\]
...
L'\textit{exactitude} compare la réponse à une vérité de terrain lorsque celle-ci existe :
\[
\operatorname{Correctness}(A_i,Y_i)=h(A_i,Y_i),
\]
...
\subsection{Construction du jeu de test et optimisation orientée utilisateur}
...
\[
\mathcal{C}_1,\ldots,\mathcal{C}_m = \operatorname{Cluster}\big(f_\theta(q_1),\ldots,f_\theta(q_N)\big).
\]
...
Des bibliothèques comme RAGAS proposent des métriques automatisées pour les pipelines \RAG{}, notamment la fidélité, la pertinence de la réponse, la précision du contexte et le rappel du contexte \cite{es2024ragas}. Leur intérêt est de rendre les itérations comparables : après une modification du chunking, du retriever hybride, du prompt ou du modèle génératif, les scores doivent être recalculés sur le même jeu de test. Une amélioration n'est crédible que si elle augmente les scores sur les cas importants sans dégrader les cas sensibles.
