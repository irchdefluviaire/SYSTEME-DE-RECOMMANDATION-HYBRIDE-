...
La figure~\ref{fig:approche_methodologique} résume la logique du projet. La solution part des données locales et des référentiels, les transforme en données propres, produit des représentations vectorielles, construit un graphe de connaissances, puis combine ces éléments pour générer des recommandations explicables. Chaque brique joue donc un rôle précis dans le système.

\subsection{Sources de données}

La première brique correspond aux données utilisées au départ du projet. Elle regroupe les offres d'emploi locales, les profils de demandeurs, les référentiels \ESCO{}, \NCF{} et \MEPC{}, ainsi que les informations utiles sur les employeurs, les secteurs et les localisations. Ces sources permettent de représenter les deux côtés du marché du travail : d'un côté les besoins exprimés dans les offres, de l'autre les profils disponibles chez les candidats.

Cette brique est importante parce que le système ne peut recommander correctement que ce qu'il connaît correctement. Une offre mal renseignée, une compétence absente ou un niveau de formation ambigu deviennent ensuite des limites pour le graphe, les embeddings et le score final. Le projet commence donc par organiser ces données au lieu de lancer directement un modèle sur des textes bruts.

\subsection{ETL et normalisation}

La deuxième brique correspond au traitement réalisé par le pipeline ETL, notamment à travers le script \texttt{scripts/run\_etl.py}. Son rôle est de transformer des données hétérogènes en fichiers exploitables par les modules suivants. Le pipeline normalise les offres, normalise les candidats, aligne les référentiels \MEPC{}, \NCF{} et \ESCO{}, puis construit les paires utilisées pour le fine-tuning.

Concrètement, cette étape nettoie les textes, harmonise les libellés, structure les champs importants, extrait ou organise les compétences, rapproche les niveaux de formation et produit des fichiers de sortie dans \texttt{data/processed} et \texttt{data/finetune}. C'est la base technique du mémoire : si cette étape est faible, tout le reste devient fragile, car le modèle et le graphe travailleront sur des données mal stabilisées.

\subsection{Fine-tuning du Sentence Transformer}

La troisième brique correspond à l'adaptation du modèle d'embeddings. Le projet utilise un modèle Sentence Transformer, puis l'adapte au vocabulaire des offres locales à partir de paires construites pendant l'ETL. Cette étape est portée par le module \texttt{src/02\_finetune\_st}. L'objectif n'est pas de créer un grand modèle de langage depuis zéro, mais d'améliorer la représentation vectorielle des textes du domaine emploi-compétences.
...
La quatrième brique correspond au chargement du graphe de connaissances dans Neo4j avec le module \texttt{src/03\_knowledge\_graph/load\_neo4j.py}. Le graphe représente les entités utiles au raisonnement : candidats, offres, compétences, métiers, niveaux \NCF{}, groupes \MEPC{}, secteurs, employeurs et localisations.
...
\subsection{Base vectorielle PostgreSQL et pgvector}

La cinquième brique correspond à l'indexation vectorielle dans PostgreSQL avec l'extension pgvector. Le module \texttt{src/04\_pgvector/embed\_all\_entities.py} encode les entités du projet avec le modèle d'embeddings, puis stocke les vecteurs dans la base. Cette brique permet d'effectuer une recherche rapide par similarité sémantique.
...
\subsection{Ingestion des PDF réglementaires}

La sixième brique concerne les documents officiels utilisés dans le projet, notamment les PDF liés à la \NCF{}, à la \MEPC{} et aux diplômes. Le script \texttt{scripts/ingest\_pdfs.py} découpe ces documents en chunks, produit leurs embeddings, les indexe dans pgvector et les relie au graphe lorsque les informations correspondantes sont détectées.
...
\subsection{Moteur de recommandation hybride}

La septième brique correspond au moteur GraphRAG du projet. Ce moteur charge un candidat, construit un contexte à partir de pgvector et de Neo4j, puis produit des recommandations, un \emph{skill gap} et une roadmap.
...
\subsection{Orchestration, API et interfaces}

La huitième brique rend le système exploitable. Le projet contient une API FastAPI dans \path{src/06_api}, un workflow agentique LangGraph dans \path{src/08_agentic_graphrag}, une interface Streamlit avec \path{chatbot_app.py} et une CLI interactive. Ces éléments servent à interroger le système, afficher les traces du raisonnement et rendre les recommandations accessibles à un utilisateur.
...
\subsection{Sorties et évaluation}

La dernière brique correspond aux résultats produits et à leur contrôle. Le projet prévoit des recommandations d'offres, du matching candidat--emploi, une analyse du \emph{skill gap}, une roadmap de formation, ainsi que des rapports et fichiers d'évaluation dans \texttt{outputs/evaluation}. Le module \texttt{src/07\_evaluation} sert à évaluer différents aspects du système, comme les embeddings, le GraphRAG, les scores et la latence.

Cette étape est nécessaire pour ne pas juger le système uniquement sur son architecture. Une méthodologie sérieuse doit observer les sorties : quelles offres sont recommandées, pourquoi elles le sont, quelles compétences manquent, quelles sources sont mobilisées et quelles limites apparaissent dans les métriques. C'est cette brique qui permet de passer d'une démonstration technique à une analyse défendable dans le mémoire.
