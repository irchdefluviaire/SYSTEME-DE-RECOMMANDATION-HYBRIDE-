...
\tcbset{
  ...
  problematiquebox/.style={
    colback=blue!2,
    colframe=midblue,
    coltitle=white,
    fonttitle=\bfseries,
    title=Question centrale de recherche,
    boxrule=1pt,
    arc=2mm,
    left=3mm,
    right=3mm,
...
\textit{\textbf{Communauté Économique et Monétaire de l'Afrique Centrale}}\\
 (CEMAC)\\[.05cm]
\includegraphics[scale=0.08]{figures/40ansISSEA.png}
 \end{center}
\begin{center}
\textit{\textbf{Institut Sous-régional de Statistique\\
		 et d'Économie Appliquée}}\\[0.05cm]
...
\begin{minipage}[t]{0.47\textwidth}
   \begin{center}
\textit{\textbf{Cameroun AI Enthousiaste}}\\[0.05cm]
\includegraphics[scale=0.6]{figures/kmerai.png}
 \end{center}
\begin{center}
\textit{\textbf{Direction du Développement et de la Recherche}}\\[0.05cm]
	{\underline{\textit{\textbf{ONG camerounaise}}}}\\
BP : 83000 Toulon, France\\
\textit{Site web : https://kmer-ai.org/}
\end{center}
 \end{minipage}
\vspace*{0.03cm}
\begin{center}
	\normalsize {\textbf{\textit{Mémoire professionnel de fin d’étude présenté en vue de l’obtention du diplôme d’Ingénieur
Statisticien Économiste(ISE)}}}\\[0.05cm]
...
%\includegraphics[scale=0.7]{sty.png}\\
    ...
    \normalsize \textbf{\color{blue!100!black}{CONCEPTION D'UN PIPELINE HYBRIDE DE RECHERCHE SÉMANTIQUE POUR LES RECOMMANDATIONS D'EMPLOI--COMPÉTENCES, FONDÉ SUR LES LLMs ET LES GRAPHES DE CONNAISSANCES}}
    {\color{midblue}\rule[-0.002mm]{17cm}{2.2mm}}\definecolor{bordeaux}{rgb}{.4,0,0}
\par\vspace{0.5mm}
    {\normalsize\textit{Stage effectué du 10 Mars au 30 juin 2026}}\\[0.3mm]
   \tcbox[colback=blue!4, colframe=midblue, boxrule=0.8pt, arc=1mm, left=2mm, right=2mm, top=0.8mm, bottom=0.8mm]{\textbf{\normalsize Rédigé par :}}\par\vspace{0.4mm}
    {\normalsize\bfseries NGOULOU NGOUBILI Irch Defluviaire}\\
    {\normalsize Élève Ingénieur Statisticien Économiste cycle long -- $5^e$ année}\\[0.04cm]
    % \tcbox[colback=blue!4, colframe=midblue, boxrule=0.8pt, arc=1mm, left=2mm, right=2mm, top=0.8mm, bottom=0.8mm]{\textbf{\normalsize Sous l'encadrement de :}}\par\vspace{0.06cm}
...
\vspace*{0.12cm}
\begin{center}
\footnotesize\textit{\textbf{Soutenu publiquement le 07 Juillet 2026 devant le jury composé de :}}
\end{center}
\vspace*{0.05cm}
\noindent
\footnotesize
...
\begin{center}
\textbf{Examinateur}\\[0.04cm]
\textbf{Pr MELATAGIA Paulin}\\[0.03cm]
\textit{Enseignant associé à l'ISSEA}
\end{center}
\end{minipage}%
\begin{minipage}[t]{0.33\textwidth}
\begin{center}
\textbf{Président}\\[0.04cm]
\textbf{Dr WANSOUWE WANBITCHING}\\[0.03cm]
\textit{Enseignant permanent à l'ISSEA}
\end{center}
\end{minipage}%
\begin{minipage}[t]{0.33\textwidth}
\begin{center}
\textbf{Encadrante}\\[0.04cm]
\textbf{Dr FOUTSE Yuehgoh}\\[0.03cm]
\textit{Présidente de KmerAi}
\end{center}
\end{minipage}
\vfill
\begin{center}
    \tcbox[colback=midblue, colframe=midblue, boxrule=0.8pt, arc=1mm, left=3mm, right=3mm, top=1mm, bottom=1mm]{\normalsize\textbf{\textcolor{white}{Session de Juillet 2026}}}
\end{center}
\normalsize
\end{titlepage}
...
\raggedleft
\begin{spacing}{1.5}
{\Large\itshape À mes parents,}\par
\vspace{0.7cm}
{\Large\bfseries\textcolor{darkblue}{LIKIBI NGALI Bérangère}}\par
\vspace{0.15cm}
{\large\itshape et}\par
\vspace{0.15cm}
{\Large\bfseries\textcolor{darkblue}{KIVOUNDAKA Pierre}}\par
\vspace{1cm}
\rule{0.34\textwidth}{0.6pt}\par
\vspace{0.8cm}
{\itshape
Pour votre amour, vos sacrifices et les valeurs que vous m'avez transmises.\\
Votre courage, votre confiance et votre soutien ont accompagné chacun de mes pas.\\
Ce travail porte l'empreinte de vos efforts et de vos espérances.}
\par\vspace{1cm}
{\large\bfseries Je vous le dédie avec une profonde reconnaissance.}
\end{spacing}
\end{minipage}
...
\begin{spacing}{1.5}
Ce mémoire est l'aboutissement d'un travail exigeant, rendu possible par le soutien, les conseils et la disponibilité de nombreuses personnes. Je rends avant tout grâce à Dieu pour la force, la persévérance et les circonstances favorables qui m'ont permis de mener ce travail à son terme. J'adresse ensuite mes sincères remerciements à:
\begin{itemize}
	\item \textbf{Dr. FOUTSE Yuehgoh}, Présidente et Directrice exécutive adjointe de KmerAI, mon encadrante professionnelle, pour son accompagnement, sa confiance et ses orientations tout au long de ce stage ;
	\item \textbf{M. MBOU SOB Ulrich}, Directeur de la Recherche et Développement de KmerAI, pour sa disponibilité, ses remarques pertinentes et son appui dans la conduite du projet ;
	\item \textbf{Dr. Marcel OPOUMBA}, Directeur Général de l'ISSEA, pour la rigueur académique promue au sein de l'institution et pour la qualité de la formation reçue ;
	\item \textbf{Dr. Dieudonné KINKIÉLÉLÉ}, Coordonnateur de la filière ISE, pour sa rigueur, son engagement envers notre promotion et l'exemple professionnel qu'il représente ;
	\item \textbf{Mme. MBIA NDI Marie Thérèse}, mon encadrante académique, pour ses précieux conseils, ses observations constructives et le temps consacré à la relecture de ce mémoire ;
	\item à l'ensemble du corps enseignant de l'ISSEA, pour les connaissances transmises, les exigences méthodologiques et la qualité de l'encadrement reçu durant ma formation ;
	\item mon oncle, \textbf{LIKIBI LOUFOUMA Richard}, pour sa présence, son soutien constant et ses encouragements ;
	\item mes amis \textbf{DONGMO Wandji Chelsie Carel}, \textbf{KOULOU Crépin} et \textbf{ONDAY ELISA Ruth Vanessa}, pour leur disponibilité, leurs encouragements et leur accompagnement dans l'élaboration de ce travail ;
	\item la communauté des étudiants congolais de l'ISSEA (MECI), pour l'organisation des présoutenances, les séances de soutien et l'esprit de solidarité entretenu;% durant ma formation ;
	\item toutes les personnes qui ont contribué, de près ou de loin, à la réalisation de ce projet et dont les noms ne figurent pas dans ce document. Qu'elles trouvent ici l'expression de ma profonde gratitude.
\end{itemize}
\end{spacing}
...
%\vspace{-1.4cm}
\begin{spacing}{1.5}
L'Institut Sous-régional de Statistique et d'Économie Appliquée (ISSEA) est une institution spécialisée de la Communauté Économique et Monétaire de l'Afrique Centrale (CEMAC). Il assure la formation et le perfectionnement des cadres ressortissants des pays africains dans les domaines de la statistique et de l'économie appliquée.

Outre la formation continue, l'ISSEA dispose de cinq principales filières : les Ingénieurs Statisticiens Économistes (ISE classique et ISE cycle long), les Ingénieurs d'Application de la Statistique (IAS), les Analystes Statisticiens (AS), les Techniciens Supérieurs de la Statistique (TSS) et les Agents Techniques de la Statistique (ATS). Dans le cadre de ses programmes de formation continue, l'Institut propose également des formations de master et de licence professionnelle, notamment en Statistiques agricoles, en Base de données et Big Data, en Gestion et traitement statistique des données, ainsi qu'en Data Science et modélisation statistique.

Afin de permettre aux étudiants de mettre en pratique les connaissances théoriques et académiques acquises au cours de leur formation, l'ISSEA intègre des stages académiques et professionnels dans ses cursus. Dans la filière ISE cycle long, un premier stage est effectué en deuxième année pour une durée minimale de deux mois au sein d'une structure publique, parapublique ou privée. Le second stage, réalisé en cinquième année, marque la fin de la formation. D'une durée comprise entre trois et six mois, il donne lieu à la rédaction d'un mémoire professionnel en vue de l'obtention du diplôme d'Ingénieur Statisticien Économiste.

Le présent mémoire a ainsi été élaboré à l'issue d'un stage professionnel effectué à la Direction de la Recherche et Développement de KmerAI, du 10 mars au 30 juin 2026. Les travaux réalisés dans ce cadre portent sur le thème suivant :\textbf{CONCEPTION D'UN PIPELINE HYBRIDE DE RECHERCHE SÉMANTIQUE POUR LES RECOMMANDATIONS D'EMPLOI-COMPÉTENCES, FONDÉ SUR LES LLMs ET LES GRAPHES DE CONNAISSANCES}
\end{spacing}
...
\begin{spacing}{1.5}
Dans les services numériques d'emploi, le rapprochement entre offres, profils candidats et compétences est freiné par l'hétérogénéité des descriptions, la variabilité des intitulés et le manque d'explicabilité des outils fondés sur les seuls mots-clés. Dans ce contexte, ce mémoire propose et opérationnalise un pipeline de recommandation emploi--compétences par recherche hybride, fondé sur les \LLM{} et les graphes de connaissances. Le pipeline articule la normalisation des données et leur alignement avec \ESCO{}, \MEPC{} et \NCF{}, l'encodage sémantique par SentenceTransformer, le rappel top-$K$ dans PostgreSQL--pgvector, la vérification relationnelle dans Neo4j, le reranking, l'analyse du \textit{skill gap} et la génération d'explications ancrées dans les preuves récupérées. Une implémentation pilote sur 13\,957 offres, 41\,298 candidats, 72\,643 embeddings et 142 fragments documentaires documente la faisabilité technique de cette chaîne. Les diagnostics internes rapportent un Recall@10 de 0,8520 et un NDCG@10 de 0,6336 pour le retrieval sémantique ; après reranking relationnel, le NDCG@10 passe de 0,5575 à 0,7392 et le MRR@10 de 0,6908 à 0,8093. Ces chiffres ne constituent ni un taux de recrutement ni une validation métier, car ils reposent sur un proxy de pertinence. Le critic, calibré à 0,46, contrôle l'ancrage lexical de la réponse dans ses preuves sans juger la vérité métier. La contribution du mémoire est une architecture modulaire, traçable et explicable ; sa validation finale exige un jeu annoté indépendant et des tests avec des professionnels de l'emploi.

\textbf{Mots-clés :} pipeline hybride ; recherche sémantique ; recommandation emploi--compétences ; graphe de connaissances ; intelligence artificielle générative ; écart de compétences.
\end{spacing}
...
\begin{spacing}{1.5}
Digital employment services must match job offers, candidate profiles and skills described with heterogeneous vocabularies and uneven levels of detail. In this context, this thesis proposes and operationalises a job--skill recommendation pipeline based on hybrid retrieval, \LLM{}s and knowledge graphs. The pipeline combines data normalisation and alignment with ESCO, MEPC and NCF; SentenceTransformer encoding; top-$K$ semantic retrieval in PostgreSQL--pgvector; relational verification in Neo4j; reranking; skill-gap analysis; and evidence-grounded explanation generation. A pilot implementation using 13,957 job offers, 41,298 candidate profiles, 72,643 entity embeddings and 142 documentary chunks demonstrates the technical feasibility of the pipeline. The implementation and evaluation chapters report internal diagnostics: Recall@10 of 0.8520 and NDCG@10 of 0.6336 for semantic retrieval; NDCG@10 rising from 0.5575 to 0.7392 and MRR@10 from 0.6908 to 0.8093 after relational reranking. These figures are not hiring outcomes or business validation, because they rely on a relevance proxy. The generation critic, calibrated at 0.46, measures lexical grounding in retrieved evidence rather than job-domain truth. The defensible contribution is therefore a modular, explainable and auditable pipeline; independent expert annotation and user studies remain necessary for final validation.

\textbf{Keywords:} hybrid pipeline; semantic search; job--skill recommendation; knowledge graph; generative artificial intelligence; skill gap.
\end{spacing}
...
Au Cameroun, la difficulté d'insertion professionnelle ne peut pas être appréciée à partir du seul taux de chômage. Les données de la troisième Enquête sur l'Emploi et le Secteur Informel (EESI3), réalisée en 2021 par l'Institut National de la Statistique, montrent qu'environ une personne de 14 ans ou plus sur deux occupe un emploi (taux d'emploi de 50,8\,\%). Ce repère ne renseigne toutefois ni sur la qualité ni sur la stabilité de cet emploi. Dans le même temps, le taux de chômage au sens du BIT est de 6,1\,\% : il décrit le chômage ouvert, sans couvrir les personnes sous-employées ou éloignées de la recherche active d'emploi \cite{ins2023eesi3emploi}.

L'ampleur de la sous-utilisation de la main-d'œuvre complète donc ce premier diagnostic. L'indicateur SU2, qui combine chômage et sous-emploi lié au temps de travail, atteint 23,0\,\%, tandis que le SU4 atteint 26,3\,\% en y ajoutant la main-d'œuvre potentielle. Ces indicateurs sont complémentaires et ne doivent ni être additionnés ni être interprétés comme une même mesure. Pour les jeunes de 15 à 34 ans, la situation est plus défavorable : le chômage atteint 10,5\,\%, le SU2 27,3\,\% et le SU4 31,7\,\%. Cette lecture est cohérente avec la recommandation de l'Organisation Internationale du Travail (OIT) de compléter le chômage par des indicateurs plus larges, tels que le \emph{jobs gap}, afin de mieux rendre compte des personnes qui souhaitent travailler sans satisfaire toutes les conditions statistiques du chômage \cite{ilo2024jobsgap}.

L'emploi informel représente par ailleurs 86,6\,\% de l'emploi total, réparti entre l'informel non agricole (52,0\,\%) et l'informel agricole (34,7\,\%), contre 5,1\,\% pour le secteur privé formel \cite{ins2023eesi3emploi}. Dans un marché où l'informalité est prépondérante, l'enjeu n'est donc pas seulement de diffuser davantage d'offres, mais aussi de rendre les compétences, les exigences des postes et les parcours de formation comparables.

Le Fonds National de l'Emploi (FNE), service public de l'emploi du Cameroun, intervient précisément dans cette fonction d'intermédiation : information sur le marché du travail, orientation, insertion des demandeurs d'emploi et appui à l'auto-emploi \cite{fnepresentation}. En 2023, le bilan gouvernemental rapporte 53\,375 prospections d'emploi auprès des entreprises ayant conduit à 21\,719 insertions, dont 21\,503 en emploi salarié et 216 en emploi indépendant \cite{rasipefin2024}. Ces résultats constituent des flux administratifs d'insertion ; ils ne mesurent ni des créations nettes d'emplois à l'échelle nationale ni la durée des placements. Le Programme Pays de Travail Décent Cameroun 2023--2026 de l'OIT prévoit d'ailleurs le renforcement des capacités du FNE et des dispositifs d'observation à produire et diffuser l'information sur le marché du travail \cite{ilo2023pptdcameroun}.

Les plateformes d'emploi doivent rapprocher des offres, des profils et des référentiels de compétences qui ne mobilisent ni les mêmes vocabulaires ni le même niveau de détail. Les intitulés de poste sont fréquemment ambigus, les compétences sont parfois implicites ou exprimées par des synonymes, et les descriptions incomplètes limitent la pertinence de filtres exacts. Il en résulte un double risque : écarter des profils réellement compatibles et produire des recommandations dont le raisonnement reste opaque. Les modèles d'appariement montrent que ces frictions informationnelles compliquent la rencontre entre candidats et emplois \cite{mortensen1994job,pissarides2000equilibrium}.

La recherche sémantique réduit une partie de cette variabilité en rapprochant des formulations de sens voisin ; elle ne permet toutefois pas, à elle seule, de vérifier qu'un candidat détient les compétences indispensables, le niveau requis ou une trajectoire de formation cohérente. À l'inverse, un graphe de connaissances rend ces relations explicites, mais ne constitue pas seul un mécanisme de rappel efficace dans des collections textuelles volumineuses. La justification de l'étude réside donc dans leur complémentarité : la recherche hybride permet de constituer un vivier sémantiquement pertinent, puis de contrôler, classer et expliquer ce vivier au moyen des relations candidat--compétence--offre--formation.

KmerAI constitue le contexte applicatif de l'étude et la source des données pilotes. Le mémoire ne vise pas à établir un diagnostic général du marché du travail camerounais ; il vise à proposer et à opérationnaliser une chaîne de recommandation emploi--compétences reproductible, explicable et soumise au jugement humain. Cette chaîne relie la normalisation, l'indexation vectorielle, le graphe de connaissances, le reranking et la génération contrôlée par \LLM{}.
\vspace{-0.5cm}
\section*{Problématique}
\markright{Problématique}
Le problème ne se réduit pas à apparier un profil à une offre. Une recommandation exploitable doit d'abord restituer des candidats ou offres sémantiquement proches, puis vérifier les exigences explicites -- compétences, niveau, secteur ou parcours -- et enfin fournir des preuves compréhensibles. Les modèles fondés uniquement sur des mots-clés échouent sur les équivalences de formulation ; les modèles fondés uniquement sur les embeddings peuvent ignorer une compétence obligatoire ; un \LLM{} seul peut produire une explication plausible sans preuve locale.

Les référentiels \ESCO{}, \MEPC{} et \NCF{} fournissent des points d'ancrage, mais ils ne deviennent utiles qu'après normalisation, alignement et explicitation des relations dans un graphe. Le problème de recherche est donc architectural : comment articuler ces briques sans confondre rappel sémantique, contrôle relationnel, génération textuelle et décision humaine ?

La question centrale est la suivante :
\begin{center}
\begin{tcolorbox}[problematiquebox,width=0.94\textwidth]
\centering
\textit{\textbf{Comment proposer et opérationnaliser un pipeline de recommandation emploi--compétences par recherche hybride, combinant rappel sémantique, vérification par graphe de connaissances et génération contrôlée par un \LLM{} ?}}
\end{tcolorbox}
\end{center}

Cette question générale se décline en quatre questions de recherche, directement associées aux objectifs spécifiques :
\begin{enumerate}
    \item Comment normaliser les offres et les profils, puis les aligner avec les référentiels \ESCO{}, \MEPC{} et \NCF{}, afin de rendre comparables les métiers, les compétences et les niveaux de formation ?
    \item Comment concevoir une recherche sémantique fondée sur l'encodage et l'indexation vectorielle afin de retrouver, pour un profil donné, les $K$ offres candidates les plus proches sur le plan sémantique malgré les variations de vocabulaire ?
    \item Comment exploiter les relations du graphe entre candidat, offre, compétence, métier et formation afin de vérifier les exigences, d'identifier les compétences acquises et manquantes, puis de réordonner les offres de manière traçable ?
    \item Comment encadrer l'usage du \LLM{} afin que chaque explication soit fondée sur les preuves récupérées, et comment distinguer les diagnostics techniques d'une évaluation de la pertinence métier fondée sur une annotation indépendante ?
\end{enumerate}

\vspace{-0.5cm}
\section*{Objectifs spécifiques de l'étude}
\markright{Objectifs spécifiques}
L'objectif général de cette étude est de \textbf{proposer et d'opérationnaliser un pipeline de recommandation emploi--compétences par recherche hybride}. Ce pipeline combine un rappel sémantique, une vérification relationnelle et une génération contrôlée ; il constitue un outil d'aide à la décision et n'automatise ni le recrutement ni l'orientation. Plus spécifiquement, il s'agit de :
\begin{itemize}
    \item \textbf{Structurer les données d'entrée.} Définir et opérationnaliser un contrat de données permettant de normaliser les offres et les profils, puis de les aligner avec \ESCO{}, \MEPC{} et \NCF{}.
    \item \textbf{Mettre en place la recherche sémantique.} Concevoir et opérationnaliser l'encodage des textes, l'indexation vectorielle et la récupération des offres candidates les plus proches pour un profil donné.
    \item \textbf{Vérifier la compatibilité et classer les offres.} Formaliser une couche relationnelle fondée sur un graphe de connaissances afin de contrôler les exigences de l'offre, d'identifier les compétences acquises, requises et manquantes, puis de réordonner les offres de manière explicable.
    \item \textbf{Produire une explication contrôlée et préparer son évaluation.} Encadrer la génération par \LLM{} à partir des preuves récupérées et définir un protocole distinguant les diagnostics techniques, l'annotation métier indépendante et les tests utilisateurs.
\end{itemize}
Ces objectifs portent sur la conception et l'opérationnalisation du pipeline. Ils ne visent ni à mesurer un taux de recrutement ni à conclure à une utilité métier généralisable sans annotation indépendante.
%\vspace{-0.5cm}
\section*{Intérêt de l'étude }
\markright{Intérêt de l'étude}
L'intérêt de cette étude réside dans la séparation des responsabilités. La mémoire vectorielle fournit un rappel sémantique rapide ; le graphe porte les contraintes et les preuves relationnelles ; le \LLM{} n'intervient qu'après récupération pour router une requête ou formuler une explication ancrée. Cette architecture est plus défendable qu'un score opaque ou qu'une réponse générée sans source : elle rend inspectables la recommandation, le \textit{skill gap} et les limites de la donnée. Sa valeur attendue est méthodologique et transférable ; les résultats pilotes ne doivent pas être interprétés comme une mesure généralisable de l'emploi au Cameroun.
\vspace{-0.5cm}
\section*{Propositions de conception}
\markright{Propositions de conception}
Afin de répondre à la problématique, la proposition repose sur trois exigences vérifiables, et non sur des promesses d'impact métier :
\begin{itemize}
    \item \textbf{P1 :} les données doivent être normalisées et ancrées dans des référentiels avant tout rapprochement sémantique ;
    \item \textbf{P2 :} le vivier sémantique doit être contrôlé par des relations explicites entre candidat, compétences, offre et formation ;
    \item \textbf{P3 :} toute réponse générée doit être associée à ses sources, ses scores et une possibilité de révision ; elle reste soumise au jugement humain.
\end{itemize}
\vspace{-0.5cm}
\section*{Méthodologie de l'étude}
\markright{Méthodologie de l'étude}
La démarche suit une logique de conception de pipeline. Les entrées sont les offres, profils, requêtes utilisateur et référentiels. Elles sont successivement normalisées, alignées et transformées en deux mémoires complémentaires : des embeddings interrogeables pour le rappel top-$K$ et un graphe de connaissances pour les contraintes, les chemins explicatifs et le \textit{skill gap}. Le reranking utilise ces signaux sans laisser le \LLM{} décider seul du classement. Le \LLM{} route une demande, appelle des outils autorisés, puis formule une réponse à partir d'un contexte et de preuves traçables. Les chapitres~3 et~4 conservent l'implémentation pilote et l'évaluation interne de cette proposition ; leurs métriques mesurent la cohérence des sorties et non une efficacité métier généralisable. La validation finale prévue requiert un jeu annoté indépendant, une séparation stricte entre calibration et test, ainsi que des évaluations par recruteurs, conseillers et candidats.
\vspace{-0.5cm}
\section*{Plan du travail}
\markright{Plan du travail}
Le mémoire est organisé en deux parties comprenant cinq chapitres. La première établit les fondements théoriques et spécifie le pipeline, de la préparation des données à l'orchestration contrôlée. La seconde documente son implémentation pilote et évalue ses comportements internes avant une discussion critique des résultats, des limites et du protocole de validation métier à venir. Une conclusion générale rappelle enfin les apports de la proposition, ses conditions de validité et ses perspectives.
\part{CADRE THÉORIQUE ET EXPÉRIMENTAL}
...
\begin{figure}[H]
  ...
  \source{Nos travaux, schématisation conceptuelle à partir de Vaswani et al. (2017).}
\end{figure}
%\begin{align}
...
Ce chapitre spécifie le pipeline proposé et les données qui l'alimentent. Il décrit la transformation des sources en représentations, puis les règles qui organisent leur interrogation ; l'implémentation pilote et les diagnostics internes correspondants sont conservés dans les chapitres suivants.

\section{Cadre méthodologique et sources de données}
Cette première section fixe le cadre de travail. Le pipeline est conçu comme un artefact d'aide à la décision, pas comme un modèle isolé ni comme un dispositif de décision automatique.

\subsection{Démarche d'ingénierie appliquée retenue}

Le pipeline suit une chaîne de rôles séparés. Les offres, les profils et les référentiels sont d'abord collectés, nettoyés et harmonisés. Les informations comparables -- métiers, compétences, niveaux de formation et localisations -- sont ensuite représentées dans une mémoire vectorielle et un graphe de connaissances. La mémoire vectorielle produit un vivier d'offres proches ; le graphe vérifie les compétences communes et manquantes ; le reranking classe les résultats ; le \LLM{} formule enfin une explication à partir des preuves.
Cette organisation répond à quatre exigences. Premièrement, chaque recommandation doit être reliée aux données utilisées. Deuxièmement, le classement ne doit pas dépendre d'un seul indice. Troisièmement, le pipeline doit signaler les compétences acquises et celles qui manquent. Enfin, la réponse rédigée doit rester fidèle aux informations retrouvées \cite{lewis2020rag,es2024ragas}. LangGraph coordonne ces étapes dans un graphe d'états et conserve la trace des outils appelés \cite{langgraphdocs2026}. Si les informations récupérées sont insuffisantes, la règle est de rechercher à nouveau ou de signaler une information insuffisante, jamais de compléter librement par une réponse non sourcée.

La figure~\ref{fig:workflow_methodologique_simplifie} montre que le système ne traite pas toutes les questions de la même manière. Après réception de la demande, l'agent identifie le besoin de l'utilisateur et choisit le parcours adapté : recommander des offres à un candidat, consulter un référentiel, interroger les relations du graphe ou produire une vue d'ensemble. Cette étape évite d'appeler inutilement toutes les sources pour chaque question.
\IfFileExists{simplifie.png}{%
\begin{figure}[H]
  \centering
  \caption{Pipeline hybride proposé de recherche sémantique et de recommandation emploi--compétences}
  \label{fig:workflow_methodologique_simplifie}
  \includegraphics[width=0.92\textwidth,height=0.58\textheight,keepaspectratio]{vrai.png}
  \source{Nos travaux, synthèse du workflow méthodologique.}
\end{figure}
}{}
...
Dans le pipeline proposé, les éléments récupérés sont réunis dans un contexte commun. Pour une recommandation, le système rapproche d'abord le profil des offres, puis vérifie les compétences et les niveaux de formation avant de calculer les écarts. Pour une question documentaire, il récupère les passages utiles et leurs sources. Pour une question relationnelle, il utilise les liens du graphe. Le modèle de langage rédige enfin la réponse à partir de ces éléments. Un module de contrôle, appelé \textit{critic}\footnote{Le \textit{critic} vérifie si la réponse générée est suffisamment appuyée par les informations récupérées. Il ne remplace pas une validation humaine.}, décide si la réponse peut être acceptée, révisée une fois ou accompagnée d'un message d'information insuffisante. Le workflow se termine par l'affichage de la réponse dans l'API, le chatbot ou l'interface en ligne, avec les traces utiles à sa vérification.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{p{3.0cm}p{3.4cm}X}
    \rowcolor{darkblue}
    \tabheader{Données mobilisée} & \tabheader{Nature} & \tabheader{Utilisation dans la méthode} \\
    \midrule
    Offres d'emploi & Données opérationnelles collectées dans le corpus du projet à partir de plateformes d'emploi, dont la plateforme du FNE & Normalisation, extraction de compétences, génération de \filevar{text\_to\_embed}, indexation vectorielle, création des noeuds d'offres et calcul des scores de matching. \\
...
Il ne s'agit pas d'une formule universelle de pertinence. La littérature sur l'évaluation du retrieval demande de rendre explicites les jugements de pertinence et permet d'utiliser des gains gradués pour comparer des classements \cite{thakur2021beir,muennighoff2022mteb} ; elle ne fixe pas les coefficients $0{,}45$, $0{,}30$ et $0{,}25$. Ces coefficients sont donc des choix de conception transparents : le recouvrement lexical reçoit le poids principal car il est observé directement dans les métadonnées normalisées ; l'alignement sectoriel et le niveau \NCF{} jouent un rôle complémentaire de contrainte. Le proxy doit être lu comme un label faible de cohérence interne, non comme une vérité terrain.
...
\noindent où $Q_{0{,}70,c}$ est le 70\ieme{} percentile des scores du vivier du candidat. Le seuil relatif évite qu'un même seuil absolu traite identiquement des viviers dont les scores ont des distributions différentes. Il reste néanmoins un paramètre de conception ; une analyse de sensibilité des poids, des paliers \NCF{} et du percentile doit précéder toute conclusion métier. Ce protocole est reproductible, mais il mesure une cohérence interne avec les métadonnées disponibles, pas une validation métier définitive.
...
À chaque essai, Optuna propose $w_{sem}$, déduit $w_{Neo4j}=1-w_{sem}$, recalcule les scores, rerange les offres et évalue le classement. Trois objectifs sont comparés : NDCG@10, moyenne NDCG@10--Recall@10--Precision@10, et moyenne MRR@10--Precision@10. Les poids obtenus sont donc optimaux dans ce protocole expérimental, non des paramètres définitifs du marché du travail. L'analyse de \textit{skill gap} complète le score en distinguant compétences acquises, requises et manquantes. Elle transforme le classement en verdict opérationnel : compatibilité immédiate, compatibilité après montée en compétence, vivier ou profil hors cible.
...
Deux usages du \LLM{} sont distingués. Text2Cypher transforme certaines questions relationnelles en requêtes Cypher, sous contrainte du schéma Neo4j. La génération finale formule ensuite une réponse française à partir du contexte récupéré. Cette séparation est importante : le \LLM{} ne décide pas seul de la pertinence. %Il interroge ou rédige à partir des données disponibles.

\subsection{Modes d'accès et traçabilité des décisions}
Trois modes d'accès sont prévus : une interface en ligne de commande pour les tests techniques, une démonstration conversationnelle avec Streamlit et une interface de programmation FastAPI pour l'intégration applicative. Ces interfaces exposent le même moteur. La traçabilité repose sur les traces du workflow, les identifiants des entités, les sources documentaires, les scores et les chemins du graphe. Une recommandation sans preuve n'est pas défendable dans un contexte d'orientation ou de recrutement.
\subsection{Protocole d'évaluation prévu du système}
L'évaluation prévue sépare quatre niveaux : retrieval vectoriel, validation fonctionnelle du graphe, comparaison des classements et contrôle de la génération. Les métriques de ranking retenues incluent Precision@K, Recall@K, MRR@K, MAP@K et NDCG@K \cite{thakur2021beir,muennighoff2022mteb}. Cette séparation évite une confusion classique dans les systèmes \RAG{} : une réponse bien formulée ne prouve pas que les bonnes informations ont été retrouvées ; inversement, un bon retrieval ne garantit pas une bonne synthèse.

\section*{Conclusion}
Ce chapitre a transformé la problématique en spécification de pipeline. Les données d'offres, de candidats et de référentiels sont d'abord normalisées, puis représentées dans deux mémoires complémentaires : pgvector pour le rappel sémantique et Neo4j pour les relations métier--compétence--formation. Le score hybride, la mesure du \textit{skill gap} et l'orchestration agentique structurent ensuite la production de recommandations traçables. La méthodologie retient aussi une règle de prudence : les métriques de ranking et le critic évaluent la cohérence interne du prototype, mais ne remplacent pas une validation humaine métier. Le chapitre suivant documente l'opérationnalisation pilote de cette architecture et les choix techniques réalisés sous les contraintes de données et de ressources disponibles.
...
\part{PRÉSENTATION DES RÉSULTATS ET DISCUSSION}
...
\begin{table}[H]
  ...
  \begin{tabularx}{0.82\textwidth}{p{5.0cm}rX}
    ...
    \midrule
    \filevar{OFFRE\_EMPLOI} & 13\,957 & Corpus principal de recommandation d'offres. \\
    ...
    \filevar{CANDIDAT} & 41\,298 & Recherche candidat--offre et comparaison de profils. \\
    \filevar{GROUPE\_BASE\_MEPC} & 209 & Ancrage métier national. \\
    \filevar{DOMAINE\_DETAILLE\_NCF} & 201 & Ancrage formation national. \\
    \midrule
    \textbf{Total entités structurées} & \textbf{72\,643} & Embeddings d'entités hors chunks documentaires. \\
    \bottomrule
  \end{tabularx}
  \tableStyleEnd
  \source{Nos travaux, traitements Python de nettoyage et de diagnostic, à partir des données collectées du projet.}
...
La documentation interactive de cette API est reportée en annexe à la figure~\ref{fig:api_fastapi_endpoints}. Elle montre que l'exposition applicative suit le même workflow que le système interne. Les routes \filevar{/recommend/} et \filevar{/recommend/candidat/\{candidat\_id\}} appellent le moteur de recommandation pour combiner la récupération vectorielle et le score issu du graphe. La route \filevar{/skill-gap/} sert à comparer les compétences d'un candidat avec celles attendues par une offre. La route \filevar{/embed/} vérifie l'encodage des textes en vecteurs de dimension 384. Enfin, les routes \filevar{/offre/} et \filevar{/offre/\{offre\_id\}} donnent accès aux informations d'offres nécessaires à l'interprétation des résultats.
...
\chapter{ÉVALUATION DE LA PERFORMANCE INTERNE DU SYSTÈME}
...
Les métriques de retrieval sont calculées sur les premières offres du classement. La notation \og @10 \fg{} signifie que l'évaluation porte sur les dix offres considérées comme les plus compatibles avec chaque candidat. Precision@10 mesure la part de bonnes offres dans cette liste de dix ; Recall@10 mesure la part des offres pertinentes du vivier effectivement retrouvées ; NDCG@10 vérifie si les meilleures offres sont placées en tête ; MRR@10 dépend du rang de la première bonne offre ; HitRate@10 indique seulement si le top 10 en contient au moins une. Ces métriques sont moyennées sur les candidats évalués et varient entre 0 et 1 : plus elles sont proches de 1, meilleur est le classement.
...
Le modèle fine-tuné atteint un Recall@10 de 0,8520, contre 0,3192 pour MiniLM-L6 généraliste. Cela signifie que, sur l'ensemble des requêtes de test, environ 85,2\,\% des éléments attendus sont retrouvés dans les dix premiers résultats. Le NDCG@10 passe de 0,1980 à 0,6336 : l'ordre obtenu représente 63,4\,\% de la qualité du classement idéal sur ce test. Cette mesure porte sur la position des résultats, pas sur une probabilité de recrutement. La latence moyenne reste contenue à 32,77 ms par phrase, ce qui reste compatible avec une recherche interactive locale.
...
\includegraphics[width=0.92\textwidth]{figures/generated/evaluation/embedding_benchmark_plot.png}
\source{Nos travaux, évaluation SentenceTransformer sur le jeu de test offre--profil.}
\end{figure}
...
\toprule
Configuration & Precision@10 & Recall@10 & NDCG@10 & MRR@10 & HitRate@10 \\
\midrule
pgvector seul & 0.4623 & 0.4491 & 0.5575 & 0.6908 & 0.9420 \\
pgvector + graphe & 0.6087 & 0.6147 & 0.7392 & 0.8093 & 0.9420 \\
\midrule
Différence & +0.1464 & +0.1656 & +0.1817 & +0.1185 & 0.0000 \\
\bottomrule
\end{tabular}
\source{Nos travaux, ablation du moteur de recommandation sur 69 candidats évalués.}
\end{table}

L'ajout de Neo4j améliore toutes les mesures du top 10 sauf le HitRate, qui reste stable. Une Precision@10 de 0,6087 signifie qu'en moyenne environ 6 des 10 offres affichées après reranking sont jugées pertinentes par le proxy, contre environ 4,6 avec pgvector seul. Le Recall@10 de 0,6147 signifie que le top 10 récupère près de 61,5\,\% des offres pertinentes présentes dans le vivier évalué. Le NDCG@10 de 0,7392 indique que l'ordre obtenu atteint 73,9\,\% de la qualité du classement idéal défini par ce proxy. Enfin, un MRR@10 de 0,8093 montre que la première offre pertinente apparaît généralement très près du début de la liste. Ces valeurs décrivent une cohérence interne avec le proxy de pertinence ; elles ne signifient pas que 73,9\,\% des recommandations seraient validées par des recruteurs.

La lecture correcte est séquentielle. pgvector constitue d'abord un vivier de 30 offres plausibles. Neo4j départage ensuite ces offres à partir des relations \filevar{POSSEDE}, \filevar{REQUIERT}, \filevar{A\_NIVEAU} et \filevar{REQUIERT\_NIVEAU}. L'amélioration mesurée vient donc du nouvel ordre appliqué au même vivier, et non d'un changement des offres disponibles.
\begin{figure}[H]
\centering
\caption{Effet de l'enrichissement graphe sur les métriques de retrieval}
\label{fig:eval_ablation_metrics}
\includegraphics[width=0.92\textwidth]{figures/generated/evaluation/ablation_metrics_comparison.png}
\source{Nos travaux, ablation du moteur de recommandation sur 69 candidats évalués.}
\end{figure}
...
La figure doit être lue comme quatre diagnostics distincts, et non comme un score global. En haut, elle vérifie que le graphe permet de passer d'un métier à ses compétences et d'une compétence aux offres associées. En bas à gauche, elle montre la distribution des compétences manquantes par couple candidat--offre. En bas à droite, elle mesure la part des couples dont les niveaux \NCF{} correspondent exactement. Elle contrôle donc le fonctionnement des chemins Neo4j ; elle ne prouve pas à elle seule la pertinence finale des recommandations.
\begin{figure}[H]
\centering
\caption{Diagnostics fonctionnels des chemins Neo4j mobilisés par le GraphRAG}
\label{fig:eval_functional_graph}
\includegraphics[width=0.96\textwidth]{figures/generated/evaluation/functional_graph_evaluation.png}
...
\includegraphics[width=0.92\textwidth]{figures/generated/evaluation/graph_rerank_rank_shift.png}
...
\subsection{Pondérations optimales avec Optuna (optimisation bayésienne)}
...
La calibration explore un poids sémantique $w_{sem}\in[0,1]$ et pose $w_{Neo4j}=1-w_{sem}$ ; 500 essais Optuna recherchent ainsi un compromis sur une seule dimension. L'unité pertinente est le candidat et son top~10, non un couple candidat--offre isolé : les mesures doivent être agrégées par candidat pour éviter qu'un candidat ayant davantage de couples ne pèse indûment dans l'objectif. Cette étape est une calibration interne, pas un test externe : le même protocole doit ensuite être rejoué sur des candidats tenus à l'écart et, idéalement, sur des annotations métier indépendantes.
...
Le résultat est clair mais doit être bien défendu. Un NDCG@10 de 0,9697 signifie que, selon le proxy utilisé pour cette calibration, l'ordre du top 10 atteint 96,97\,\% de l'ordre idéal. Ce n'est ni une précision de 96,97\,\%, ni un taux de recrutement réussi. Lorsque l'objectif privilégie l'ordre global du top 10, Optuna retient un score Neo4j pur : $w_{sem}=0$ et $w_{Neo4j}=1$. Cela ne signifie pas que pgvector est inutile : pgvector a déjà construit le pool de candidats à reclasser, de sorte que l'optimum ne porte que sur la seconde étape. Il peut aussi refléter un alignement partiel entre le proxy de pertinence et les métadonnées de couverture exploitées par Neo4j. Lorsque l'objectif privilégie davantage la première bonne recommandation, le compromis devient hybride avec $w_{sem}=0{,}6615$ et $w_{Neo4j}=0{,}3385$. La solution de bord doit donc être analysée par stabilité des poids, par bootstrap ou validation croisée au niveau candidat, puis sur un jeu séparé ; elle n'est pas un paramètre universel du pipeline.
...
\includegraphics[width=0.10\textwidth]{figures/generated/evaluation/hybrid_weights_multi_objective_comparison.png}
\includegraphics[width=0.10\textwidth]{figures/generated/evaluation/hybrid_weights_ndcg_comparison_ndcg.png}
...
\includegraphics[width=0.90\textwidth]{figures/generated/evaluation/critic_contrastive_distributions.png}
...
\includegraphics[width=0.90\textwidth]{figures/generated/evaluation/critic_contrastive_threshold_grid.png}
\source{Nos travaux, évaluation du critic à partir des réponses générées et des contextes récupérés.}
\end{figure}
La valeur 0,46 est retenue parce qu'elle maximise le compromis F1 dans ce test tout en respectant une contrainte de prudence : ne laisser passer aucun contexte mélangé. À ce niveau, la matrice de décision est $TP=50$, $FN=1$, $TN=51$ et $FP=0$, où la classe positive désigne une réponse associée à son vrai contexte. La sensibilité vaut donc $50/51=98{,}04\,\%$, la spécificité $51/51=100\,\%$, la précision $100\,\%$ et $F1=100/101=99{,}01\,\%$. Ces valeurs décrivent uniquement la capacité à distinguer le vrai contexte du contexte mélangé dans cet échantillon.

La figure montre l'arbitrage du seuil. Un seuil trop bas accepte trop facilement des réponses peu ancrées ; un seuil trop haut force des révisions inutiles. Le seuil 0,46 se situe dans la zone où les contextes mélangés sont rejetés tout en conservant presque toutes les réponses associées à leur vrai contexte. Le choix est donc une règle de prudence opérationnelle, mais l'absence observée de fausse acceptation sur 51 cas ne prouve pas que son taux réel est nul.

Cette calibration ne transforme pas le critic en juge parfait. Une réponse acceptée n'est pas automatiquement correcte au sens métier ; elle est seulement suffisamment ancrée lexicalement dans les preuves récupérées. En outre, le même échantillon a servi à sélectionner puis à décrire le seuil : la performance rapportée est donc optimiste. Une calibration expérimentale complète doit séparer les requêtes de développement, de validation et de test, conserver les paires réponse--contexte ensemble lors du découpage, puis fournir des intervalles d'incertitude par bootstrap et une annotation humaine de la factualité. Pour défendre le système devant un jury, la formulation correcte est donc : le seuil 0,46 est un seuil opérationnel calibré empiriquement par contraste, en attente d'une validation indépendante de la qualité réelle des recommandations générées.
\section*{Conclusion}
...
\chapter{SYNTHÈSE ET DISCUSSION DES RÉSULTATS}

Ce chapitre relie les résultats techniques aux hypothèses de recherche formulées en introduction. Il ne reprend pas tous les tableaux d'évaluation ; il met en évidence les enseignements majeurs du mémoire, les confronte à la littérature et précise la portée réelle des résultats obtenus.

\section{Résultats majeurs de l'étude}

Le premier résultat important concerne la qualité de la représentation sémantique. Le modèle SentenceTransformer adapté au domaine emploi--compétences atteint un Recall@10 de 0.8520 et un NDCG@10 de 0.6336. Cette progression confirme que le vocabulaire des offres, des profils et des compétences ne peut pas être correctement traité par une représentation générique seule. Les intitulés de poste, les compétences opérationnelles et les descriptions de formation possèdent des régularités spécifiques ; le fine-tuning permet de rapprocher les objets qui se correspondent réellement dans ce domaine.

Le deuxième résultat porte sur l'apport de Neo4j. L'ablation montre que le passage de pgvector seul à pgvector enrichi par le graphe améliore le NDCG@10 de 0.5575 à 0.7392 et le MRR@10 de 0.6908 à 0.8093. Ce gain est central : pgvector fournit un vivier sémantiquement plausible, mais Neo4j permet de départager les offres à partir de relations métier observables, notamment les compétences possédées, les compétences requises et les niveaux de formation.

Le troisième résultat concerne l'explicabilité. Le système ne produit pas uniquement une liste d'offres ; il identifie les compétences couvertes, les compétences manquantes et les relations qui relient un candidat à une offre. Cette capacité est importante pour un outil de décision : une recommandation emploi--compétences ne doit pas seulement répondre à la question \og quelle offre proposer ? \fg{}, mais aussi à \og pourquoi cette offre ? \fg{} et \og que manque-t-il pour y accéder ? \fg{}.

Le quatrième résultat concerne la génération contrôlée. Le critic calibré à 0,46 sépare correctement les réponses ancrées dans leur vrai contexte des réponses associées à un contexte mélangé. Ce résultat ne prouve pas que toutes les réponses sont parfaites au sens métier, mais il montre que le système dispose d'un mécanisme de prudence avant de présenter une réponse finale.

\section{Confrontation avec la littérature}

Les résultats confirment d'abord les limites des systèmes de recommandation fondés uniquement sur les interactions passées. Les méthodes de filtrage collaboratif et de factorisation matricielle sont efficaces lorsque les historiques d'interaction sont riches \cite{koren2009matrix,he2017ncf}. Dans le cas étudié, cette condition n'est pas pleinement remplie : les profils et offres disponibles ne forment pas une matrice dense de clics, candidatures ou recrutements validés. Le recours à une recommandation hybride est donc cohérent avec la littérature qui recommande de combiner plusieurs signaux lorsque les données sont hétérogènes \cite{burke2002hybrid,ricci2011recommender}.

Les résultats rejoignent également les travaux sur les embeddings de phrases. Sentence-BERT montre l'intérêt de représentations adaptées aux tâches de similarité sémantique \cite{reimers2019sbert}. Dans ce mémoire, le fine-tuning confirme cette idée sur un cas appliqué : l'adaptation au vocabulaire emploi--compétences améliore le retrieval et rend pgvector plus utile comme mémoire de rappel.

L'apport du graphe s'inscrit dans la logique des graphes de connaissances, qui structurent les entités et leurs relations \cite{hogan2021knowledge}. La littérature RAG montre que la génération gagne en fiabilité lorsqu'elle s'appuie sur des sources récupérées plutôt que sur la seule mémoire paramétrique du modèle \cite{lewis2020rag,gao2023ragllm}. Le GraphRAG utilisé ici prolonge cette logique : les preuves ne sont pas seulement des textes proches, mais aussi des relations métier--compétence--formation exploitables.

Enfin, la présence du critic rejoint les approches récentes qui cherchent à contrôler l'ancrage des réponses générées \cite{asai2024selfrag,es2024ragas}. Le résultat obtenu doit toutefois rester correctement interprété : le critic mesure surtout la cohérence entre la réponse et le contexte récupéré ; il ne remplace pas l'expertise humaine sur la pertinence finale d'une recommandation.

\section{Discussion des hypothèses de recherche}

Le tableau ci-dessous résume la lecture des hypothèses au regard des résultats obtenus. Le statut retenu reste prudent : une hypothèse peut être soutenue par les résultats internes sans être définitivement validée au sens métier.

\begin{table}[H]
\centering
\caption{Lecture des hypothèses de recherche au regard des résultats}
\label{tab:synthese_hypotheses}
\tableStyleStart
\begin{tabularx}{\textwidth}{p{1.2cm}p{4.9cm}p{3.1cm}X}
\rowcolor{darkblue}
\tabheader{Hyp.} & \tabheader{Idée testée} & \tabheader{Statut} & \tabheader{Justification synthétique} \\
\midrule
H1 & Données structurées et modèle adapté au vocabulaire de l'emploi & Soutenue partiellement & Le modèle adapté améliore nettement le classement sémantique et les référentiels harmonisent les métiers, compétences et niveaux. L'effet propre de chaque référentiel n'a toutefois pas été isolé. \\
H2 & Apport des relations entre candidats, compétences et offres & Soutenue & À dix résultats, le NDCG passe de 0,5575 à 0,7392 et le MRR de 0,6908 à 0,8093 après l'ajout du graphe. \\
H3 & Pertinence et compréhension de la recommandation complète & Soutenue avec prudence & Le workflow combine recherche, relations et contrôle de la réponse, puis indique les compétences couvertes et manquantes. Une validation par des experts métier reste nécessaire. \\
\bottomrule
\end{tabularx}
\tableStyleEnd
\source{Nos travaux, synthèse des résultats des chapitres 3 et 4.}
\end{table}

La lecture globale est favorable au système hybride, sans permettre une conclusion définitive sur son usage en recrutement. H2 est la mieux étayée, car elle repose sur une comparaison directe entre deux classements appliqués au même vivier d'offres. H1 combine un gain quantitatif du modèle adapté et un apport structurel des référentiels, dont l'effet séparé reste à mesurer. H3 est cohérente avec les contrôles réalisés, mais dépend encore d'une validation par des utilisateurs et des experts métier.

\section{Portée, limites et conditions d'usage}

La première limite concerne la validation humaine. Les métriques de ranking, le proxy de pertinence et le critic donnent une évaluation interne utile, mais ils ne remplacent pas une annotation indépendante par des recruteurs, des conseillers d'orientation ou des experts métier. Cette limite ne rend pas l'étude inutile ; elle précise simplement son niveau de maturité. Le mémoire valide une architecture et ses effets internes, pas encore un déploiement métier définitif.

La deuxième limite concerne les données. Les offres proviennent de sources scrapées telles que les plateformes d'emploi disponibles, avec des niveaux de complétude variables selon les champs. Le pipeline ETL réduit ce bruit par normalisation, mais il ne peut pas créer une information absente dans la source initiale. Les recommandations restent donc dépendantes de la qualité des compétences, niveaux, secteurs et descriptions collectés.

La troisième limite concerne le graphe. Les diagnostics montrent une dépendance à certains hubs de compétences très fréquents. Le score Neo4j a donc été construit à partir de la couverture des compétences requises plutôt qu'à partir du degré brut. Ce choix limite le bruit de popularité, mais il suppose que les relations \filevar{POSSEDE} et \filevar{REQUIERT} soient correctement extraites et maintenues.

La quatrième limite concerne la génération. Le critic réduit le risque de réponses mal ancrées, mais il ne juge pas toute la vérité métier. Une réponse peut être fidèle au contexte récupéré tout en restant discutable si le contexte lui-même est incomplet. L'usage professionnel du système doit donc conserver une logique d'aide à la décision, et non de décision automatique.

En définitive, l'étude montre qu'un système hybride est plus défendable qu'une approche uniquement textuelle ou uniquement générative. Sa valeur tient à l'articulation entre trois couches : une mémoire vectorielle pour retrouver rapidement des candidats plausibles, une mémoire relationnelle pour expliquer les correspondances, et une orchestration agentique pour mobiliser les bons outils selon la requête.


\chapter*{CONCLUSION GÉNÉRALE ET PERSPECTIVES}
\markboth{Conclusion générale et perspectives}{Conclusion générale et perspectives}
\addcontentsline{toc}{chapter}{Conclusion générale et perspectives}
Ce mémoire avait pour objectif de proposer et de formaliser un pipeline hybride de recherche sémantique et de recommandation emploi--compétences, capable de transformer des profils, des offres et des référentiels hétérogènes en recommandations explicables. Cette proposition est structurée autour de rôles séparés : la normalisation rend les données comparables ; la mémoire vectorielle fournit le rappel sémantique ; le graphe de connaissances vérifie les relations métier--compétence--formation ; le reranking et le \textit{skill gap} explicitent le classement ; enfin, le \LLM{} orchestre ou formule une réponse à partir de preuves tracées.

Les chapitres d'implémentation et d'évaluation conservent une opérationnalisation pilote de ce pipeline. Ils montrent que les briques peuvent être exécutées ensemble et que leurs sorties internes peuvent être mesurées. Le fine-tuning, le reranking relationnel et le critic constituent ainsi des diagnostics techniques utiles, mais ils ne démontrent ni un taux de recrutement réussi ni une supériorité métier généralisable. Cette distinction est essentielle : une bonne valeur de NDCG sur un proxy renseigne sur l'ordre induit par ce proxy ; elle ne remplace pas le jugement d'un recruteur ou d'un conseiller.

La contribution du mémoire est donc une architecture modulaire et auditable plutôt qu'un modèle prétendument autonome. Elle défend une règle simple : une similarité élevée ne suffit pas à recommander ; la compatibilité doit être contrôlée par des relations explicites, et toute explication générée doit pouvoir être reliée à ses sources. La plateforme KmerAI constitue un contexte de démonstration de cette proposition, non une base suffisante pour généraliser des conclusions à l'ensemble du marché camerounais.

Les limites identifiées sont méthodologiquement décisives. Le proxy de pertinence partage des signaux avec le système évalué ; la calibration du critic repose sur des contextes contrastés ; les données collectées peuvent être incomplètes ou déséquilibrées. Avant tout usage métier, les poids du reranking, les seuils et les prompts doivent être évalués hors échantillon, avec des annotations indépendantes et une analyse d'incertitude.

Les perspectives suivent donc un ordre exigeant : constituer un jeu de test annoté par des recruteurs, conseillers d'orientation et experts formation ; séparer calibration et évaluation finale ; mesurer la pertinence, l'explicabilité et les erreurs par sous-groupes ; puis réaliser des tests utilisateurs encadrés. L'intégration ultérieure dans le site de KmerAI, par une interface de programmation et un chatbot, ne devra intervenir qu'après ces validations. Le graphe, les modèles d'embeddings et les artefacts de test pourront alors être versionnés et documentés afin de rendre le pipeline reproductible et maintenable.
...
%  \includegraphics[height=0.7\textheight]{figures/myWorkflow.png}
 % \source{Nos travaux, traitements Python de nettoyage et de diagnostic, à partir des données collectées du projet.}
%\end{figure}
\begin{figure}[H]
  \centering
  ...
  \includegraphics[width=0.90\textwidth]{chatbot.png}
  \source{Nos travaux, capture locale de l'interface}
\end{figure}

\begin{figure}[H]
  \centering
  \caption{Documentation interactive des endpoints FastAPI}
  \label{fig:api_fastapi_endpoints}
  \includegraphics[width=0.90\textwidth]{api1.png}
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{p{0.8cm}p{3.0cm}p{4.3cm}X}
		...
		7 & Exposition applicative & Workflow agentique et services locaux & API FastAPI, interface en ligne de commande de test et interface Streamlit. \\
		\bottomrule
	\end{tabularx}
	\tableStyleEnd
	\source{Nos travaux, traitements Python de nettoyage et de diagnostic, à partir des données collectées du projet.}
