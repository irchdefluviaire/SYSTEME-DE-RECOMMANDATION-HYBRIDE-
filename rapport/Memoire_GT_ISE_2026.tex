...
\chapter{Source de données et Approche méthodologique}
\vspace{-1.4cm}
...
Sur le plan de l'orchestration, cette étude retient explicitement \textit{LangGraph} comme cadre méthodologique de pilotage du système agentique \cite{langgraphdocs2026}. Ce choix signifie que le raisonnement applicatif n'est pas laissé à une boucle conversationnelle informelle. LangGraph est donc utilisé pour organiser le routage entre les briques du système, conserver les traces des outils mobilisés, permettre une révision lorsque le contexte est insuffisant et rendre le workflow auditable.
...
\begin{figure}[p]
  ...
  \source{Nos travaux, traitements Python de nettoyage et de diagnostic, à partir des données collectées du projet.}
  \includegraphics[height=0.88\textheight]{figures/myWorkflow.png}
\end{figure}

La figure \ref{fig:architecture_methodologique_workflow_agentique} présente l'architecture générale retenue pour la solution. Elle doit être lue comme une carte méthodologique du système plutôt que comme un simple schéma technique. Le point d'entrée est la requête de l'utilisateur. Cette requête n'est pas transmise directement au modèle génératif ; elle est d'abord interprétée par une couche d'orchestration qui détermine le type de besoin exprimé. Cette étape est centrale, car une demande de recommandation candidat-offre, une question sur un référentiel, une interrogation relationnelle sur les compétences ou une question globale ne mobilisent pas les mêmes sources ni les mêmes traitements.
...
\section{Modélisation sémantique et structuration des connaissances}
...
L'orchestration du système repose sur un workflow agentique implémentable avec \textit{LangGraph}. Méthodologiquement, LangGraph est retenu parce qu'il permet de représenter l'agent comme un graphe d'états plutôt que comme une simple succession de prompts \cite{langgraphdocs2026}. La requête utilisateur est d'abord analysée afin d'identifier son intention : diagnostic, recommandation candidat, skill gap, recherche référentielle, question relationnelle sur le graphe, orientation métier ou recherche générale. Le système sélectionne ensuite les outils adaptés. Cette étape de routage est essentielle : interroger pgvector, Neo4j ou les documents réglementaires ne répond pas au même besoin.
...
\section{Évaluation prévue, limites et synthèse méthodologique}

\subsection{Protocole d'évaluation prévu}
...
\begin{figure}[H]
  \centering
  \caption{Workflow agentique implémenté dans LangGraph}
  \label{fig:workflow_langgraph_chap3}
  \source{Nos travaux, traitements Python de nettoyage et de diagnostic, à partir des données collectées du projet.}
  \resizebox{0.95\textwidth}{!}{%
  \begin{tikzpicture}[
    node distance=0.9cm and 1.8cm,
    proc/.style={rectangle, rounded corners=3pt, draw=kmeraiteal, thick, fill=kmeraiteal!8,
      align=center, minimum width=3.2cm, minimum height=0.8cm, font=\normalsize\bfseries},
    critic/.style={diamond, draw=kmeraired, thick, fill=kmeraired!8,
      align=center, aspect=2.0, font=\normalsize\bfseries},
    arr/.style={-{Stealth[length=2.2mm]}, thick, draw=kmeraigray}
  ]
    \node[proc] (a) {\texttt{analyse\_request}};
    \node[proc, right=of a] (p) {\texttt{plan\_tools}};
    \node[proc, right=of p] (e) {\texttt{execute\_tools}};
    \node[proc, below=of e] (b) {\texttt{build\_context}};
    \node[proc, left=of b] (g) {\texttt{generate\_final\_answer}};
    \node[critic, left=of g] (c) {critic};
    \node[proc, below=of b] (x) {\texttt{expand\_context}};
    ...
    \draw[arr] (b) -- (g);
    \draw[arr] (g) -- (c);
    \draw[arr] (c) |- node[pos=0.25, above, font=\normalsize]{réviser} (x);
    \draw[arr] (x) -| (e);
  \end{tikzpicture}}
\end{figure}
...
\section{Cadre d'évaluation et protocole expérimental}

\subsection{Questions évaluées et logique de séparation}
...
L'évaluation a été conduite en cinq niveaux. Le premier niveau porte sur le modèle d'embeddings, c'est-à-dire la capacité du SentenceTransformer fine-tuné à retrouver les paires offre--profil pertinentes dans un corpus de test. Le deuxième niveau porte sur l'ablation du moteur de recommandation : une configuration \textit{pgvector seul} est comparée à une configuration \textit{pgvector + graphe}. Le troisième niveau vérifie fonctionnellement le graphe : recherche métier--compétences, compétence--offres, skill gap et compatibilité \NCF{}. Le quatrième niveau calibre les pondérations du score hybride avec Optuna. Le cinquième niveau évalue le contrôle de génération du workflow Agentic GraphRAG, en particulier le seuil \filevar{accept/revise} du critic.

Les résultats présentés dans ce chapitre proviennent d'artefacts reproductibles du pipeline local. Le benchmark des embeddings est produit par le module d'évaluation SentenceTransformer, l'ablation compare directement pgvector et Neo4j sur les mêmes requêtes, la calibration des pondérations est conduite avec Optuna, et le seuil du critic est estimé par un protocole contrastif. Cette traçabilité est importante : les nombres présentés ici ne sont pas des valeurs théoriques, mais des sorties calculées à partir des paires candidat--offre, des contextes récupérés et des réponses générées par le système.

\subsection{Données, métriques et limites du protocole}
...
Pour l'ablation \textit{pgvector} contre \textit{pgvector + graphe}, le protocole est différent. Un échantillon de 80 candidats est tiré aléatoirement avec la graine 42 ; 69 candidats ont été effectivement évalués après contrôle d'exécution. Pour chaque candidat, pgvector retourne un pool initial de 30 offres candidates. Deux classements sont alors comparés :

\begin{itemize}
    \item \textbf{pgvector seul} : l'ordre des offres est conservé tel qu'il est retourné par la recherche sémantique et lexicale dans PostgreSQL/pgvector ;
    \item \textbf{pgvector + graphe} : le même pool initial est enrichi par Neo4j, puis rerangé par le score hybride combinant similarité sémantique, compatibilité de compétences, compatibilité de niveau \NCF{} et alignement sectoriel.
\end{itemize}
...
\subsection{Performance du modèle d'embeddings fine-tuné}

Le premier résultat concerne l'intérêt du fine-tuning du SentenceTransformer. Le tableau \ref{tab:eval_embeddings} présente les performances du modèle fine-tuné par rapport à plusieurs modèles généralistes ou multilingues. Le modèle spécialisé domine nettement les alternatives sur les métriques de retrieval.
...
La figure \ref{fig:eval_ablation_metrics} confirme cette lecture. La configuration \textit{pgvector + graphe} domine la configuration vectorielle seule sur les métriques principales de classement. Cela ne signifie pas que pgvector devient secondaire : pgvector reste le premier étage, chargé de produire rapidement le pool candidat. Le résultat indique plutôt que, dans ce pool, le graphe apporte une information métier suffisamment discriminante pour améliorer le reranking.

\section{Lecture fonctionnelle du graphe Neo4j et du GraphRAG}

L'ablation précédente mesure l'effet du graphe sur le classement. Elle ne suffit pas à vérifier que Neo4j joue correctement son rôle fonctionnel. Une évaluation complémentaire a donc été réalisée avec un module dédié au diagnostic fonctionnel du graphe. Elle teste cinq scénarios directement liés aux usages du système : recherche métier vers compétences, recherche compétence vers offres, calcul de \textit{skill gap}, compatibilité \NCF{} et comparaison RAG contre GraphRAG.
...
\subsection{Recherche métier vers compétences}

Le premier test vérifie que le graphe peut partir d'un métier et récupérer les compétences associées par la relation \filevar{NECESSITE}. Ce scénario correspond à une question de type : \og quelles compétences faut-il pour exercer ce métier ? \fg{}. Les métiers les plus riches du graphe contiennent un nombre important de compétences : par exemple \og médecin spécialiste \fg{} est relié à 178 compétences, \og opérateur sur machine à commande numérique \fg{} à 158, et \og enseignant-chercheur en informatique \fg{} à 140. Sur les 25 métiers les plus riches, la moyenne atteint 124,4 compétences.

Cette richesse est utile pour l'orientation et la montée en compétences, mais elle impose une précaution : le métier porte un contexte large, pas une liste minimale d'exigences pour une offre précise. Dans le GraphRAG, les compétences métier doivent donc enrichir l'explication, tandis que les compétences \filevar{REQUIERT} d'une offre doivent rester prioritaires pour le matching opérationnel.

\subsection{Recherche compétence vers offres}

Le deuxième test part d'une compétence et récupère les offres qui la requièrent. C'est le scénario inverse : \og quelles offres demandent cette compétence ? \fg{}. La compétence \og comptabilité \fg{} est reliée à 748 offres, \og réaliser des activités de service après-vente \fg{} à 662 offres et \og planifier des campagnes de marketing sur les médias sociaux \fg{} à 626 offres. Sur les 25 compétences les plus présentes dans les offres, la moyenne atteint 590,16 offres.
...
\subsection{Skill gap}

Le \textit{skill gap} est calculé en comparant les compétences \filevar{REQUIERT} par l'offre et les compétences \filevar{POSSEDE} par le candidat. Sur l'échantillon fonctionnel, le déficit moyen est de 5,30 compétences et la médiane vaut 6. Ce résultat est cohérent avec l'objectif du système : il ne s'agit pas de supposer que tous les candidats sont proches de toutes les offres, mais d'identifier les couples où l'écart est acceptable et de transformer les autres cas en trajectoire de montée en compétences.
...
La compatibilité \NCF{} ne doit cependant pas être utilisée comme un couperet mécanique. Un écart d'un niveau peut être acceptable selon l'expérience, les compétences et la nature de l'offre. C'est pourquoi le moteur hybride l'utilise comme composante pondérée plutôt que comme filtre absolu.

\subsection{Comparaison RAG vs GraphRAG}

La comparaison RAG contre GraphRAG est matérialisée par l'ablation \filevar{pgvector seul} contre \filevar{pgvector + graphe}. Dans ce protocole, le RAG vectoriel constitue un pool initial de 30 offres par candidat. Le GraphRAG conserve le même pool, mais ajoute les relations Neo4j pour recalculer le score et reranger les offres. La différence observée ne vient donc pas d'un corpus différent, mais de l'information relationnelle ajoutée au classement.

Les résultats sont nets : le NDCG@10 passe de 0,5575 à 0,7392, le Recall@10 de 0,4491 à 0,6147, et le MRR@10 de 0,6908 à 0,8093. Le GraphRAG améliore donc l'ordre des recommandations et pas seulement leur explication. La conclusion correcte est précise : pgvector est efficace comme outil de rappel initial, tandis que Neo4j apporte un signal de décision plus discriminant lorsque les relations \filevar{POSSEDE}, \filevar{REQUIERT}, \filevar{A\_NIVEAU} et \filevar{REQUIERT\_NIVEAU} sont disponibles.

\section{Reranking hybride et calibration des poids}
...
\subsection{Pondérations optimales du score hybride avec Optuna}

Afin de ne pas conserver des pondérations arbitraires dans le moteur hybride, une étape d'optimisation a été intégrée au protocole d'évaluation. Elle exploite les sorties détaillées de l'ablation, optimise les pondérations du score hybride avec Optuna et compare trois fonctions objectif : NDCG@10, moyenne NDCG@10--Recall@10--Precision@10, et moyenne MRR@10--Precision@10. Les résultats retenus sont ensuite synthétisés sous forme de tableaux et de figures pour comparer les pondérations obtenues selon l'objectif optimisé.

L'expérience a été conduite sur 690 couples candidat--offre correspondant à 69 candidats évalués. La pertinence graduée utilisée est \filevar{proxy\_relevance}, c'est-à-dire le proxy métier construit à partir des métadonnées normalisées. Cette décision est préférable à l'utilisation directe des verdicts du moteur, car elle limite la circularité : on ne calibre pas les poids uniquement sur une étiquette déjà produite par le score hybride. Cependant, cette pertinence reste un label faible ; elle ne remplace pas une annotation humaine.
...
Le résultat est sévère pour une lecture naïve du score hybride. Lorsque la fonction objectif privilégie le classement global du top 10, Optuna retient le score Neo4j pur : $w_{sem}=0$ et $w_{Neo4j}=1$. Autrement dit, une fois que pgvector a construit le pool initial des offres candidates, le meilleur reranking selon NDCG@10 est obtenu en donnant tout le poids au taux de couverture des compétences calculé dans Neo4j. Ce résultat ne signifie pas que pgvector est inutile. Il signifie que pgvector joue surtout le rôle de retriever rapide, tandis que Neo4j devient le signal de décision le plus discriminant à l'intérieur du pool.
...
\includegraphics[width=0.82\textwidth]{figures/generated/evaluation/hybrid_weights_optimization_trials_ndcg.png}
\end{figure}

La figure \ref{fig:eval_hybrid_weights_trials} montre que les meilleurs essais se concentrent près de la frontière où le poids Neo4j domine. Cette concentration confirme que le problème n'est pas seulement un mauvais choix initial de pondérations : dans l'échantillon disponible, le signal graphe agrégé par \filevar{taux\_match} apporte une amélioration marginale au NDCG@10 lorsqu'il est utilisé pour reranger un pool déjà construit par pgvector.
...
%\begin{figure}[H]
%\centering
%\includegraphics[width=0.80\textwidth]{figures/generated/evaluation/hybrid_weights_ndcg_comparison_ndcg.png}
%\caption{Comparaison du NDCG@10 avant et après calibration}
%\label{fig:eval_hybrid_weights_comparison}
%\end{figure}

La figure \ref{fig:eval_hybrid_weights_best} confirme que le score Neo4j seul atteint un NDCG@10 de 0.9697, contre 0.9489 pour le score sémantique seul. Le score hybride actuel atteint 0.9704, légèrement au-dessus de la calibration à deux composantes, parce qu'il mobilise aussi des signaux de niveau \NCF{} et d'alignement sectoriel. La conclusion d'ingénierie est donc précise : le système doit conserver pgvector pour le rappel initial, mais le classement final doit donner une priorité forte au score graphe lorsque les relations \filevar{POSSEDE} et \filevar{REQUIERT} sont disponibles. Si l'objectif applicatif devient la meilleure première recommandation, la pondération retenue par MRR@10--Precision@10 fournit une alternative défendable avec environ deux tiers de signal sémantique et un tiers de signal graphe. La prochaine étape méthodologique consiste à calibrer simultanément toutes les composantes métier sur un jeu annoté humainement, au lieu de se limiter au couple \filevar{score\_sem}/\filevar{taux\_match}.

\section{Contrôle de génération par critic et décision accept/revise}

\subsection{Calibration contrastive du seuil de fidélité}

Le workflow Agentic GraphRAG ne s'arrête pas au classement des offres. Après le retrieval et l'enrichissement graphe, le \LLM{} OpenRouter formule une réponse finale, puis un critic local vérifie si cette réponse reste suffisamment appuyée par le contexte récupéré. Ce critic calcule un score de fidélité lexicale : il compare les mots significatifs de la réponse aux mots présents dans le contexte. La décision \filevar{accept/revise} dépend ensuite d'un seuil appliqué à ce score.

Comme aucun jeu d'annotations humaines n'est disponible à ce stade, le seuil n'a pas été calibré par vérité terrain. Une calibration contrastive non supervisée a donc été réalisée. Le principe est simple : une réponse construite à partir d'un contexte réel doit obtenir un score plus élevé que la même réponse comparée à un contexte mélangé provenant d'une autre requête. Le protocole ne mesure donc pas la satisfaction d'un recruteur ; il teste une propriété minimale du critic : rejeter les réponses lorsqu'elles ne sont plus reliées aux preuves qui les ont produites.

L'expérience utilise 51 cas de base issus des sorties fonctionnelles du graphe : recherches métier--compétences, recherches compétence--offres et indicateurs de couverture du graphe. Chaque cas est évalué deux fois : une fois avec son contexte réel, une fois avec un contexte mélangé. Le jeu contrastif contient donc 102 évaluations. Les scores moyens séparent nettement les deux situations : le score de fidélité vaut 0,7725 en moyenne avec le contexte réel, contre 0,0532 avec le contexte mélangé. La médiane passe respectivement de 0,7857 à 0,0000.
...
\source{Nos travaux, évaluation du critic, à partir des réponses générées et des contextes de preuve récupérés.}
\includegraphics[width=0.86\textwidth]{figures/generated/evaluation/critic_contrastive_distributions.png}
\end{figure}

La figure \ref{fig:eval_critic_contrastive_distributions} confirme que le critic distingue correctement les réponses appuyées par leur contexte des réponses placées face à des preuves non pertinentes. Cette séparation ne valide pas la qualité métier de chaque réponse, mais elle valide le rôle de garde-fou lexical : une réponse déconnectée de ses preuves obtient généralement un score très faible.

\begin{table}[H]
\centering
\caption{Seuil critique retenu après calibration contrastive}
\label{tab:eval_critic_threshold_contrastive}
\source{Nos travaux, évaluation du critic, à partir des réponses générées et des contextes de preuve récupérés.}
\tableStyleStart
\begin{tabularx}{0.88\textwidth}{lrrrX}
\rowcolor{darkblue}
\tabheader{Seuil} & \tabheader{F1} & \tabheader{Fausses accept.} & \tabheader{Fausses révisions} & \tabheader{Lecture} \\
\midrule
0,46 retenu & 0,9901 & 0 & 1 & Seuil prudent : il bloque les contextes mélangés dans ce test contrastif, au prix d'une révision inutile. \\
\bottomrule
\end{tabularx}
\tableStyleEnd
\end{table}

\subsection{Lecture opérationnelle des décisions accept/revise}

Le résultat le plus important est le suivant : le seuil opérationnel retenu est 0,46. À ce niveau, aucune réponse associée à un contexte mélangé n'est acceptée dans le test contrastif, tandis que 50 réponses sur 51 restent acceptées avec leur vrai contexte. La règle retenue est volontairement sévère : minimiser d'abord les fausses acceptations, puis maximiser le F1 des décisions \filevar{accept/revise}.
...
Cette calibration ne transforme pas le critic en juge parfait. Elle ne dit pas qu'une réponse acceptée est correcte au sens métier ; elle dit seulement qu'elle est suffisamment ancrée lexicalement dans les preuves récupérées. La décision correcte pour l'étude est donc de présenter 0,46 comme seuil opérationnel plus prudent pour les réponses générées, tout en conservant la nécessité d'une validation humaine future pour mesurer la qualité réelle des conseils, des citations et des trajectoires de formation.
