...
Les pondérations ne doivent pas être fixées uniquement par intuition. La méthodologie retient donc une calibration statistique par optimisation bayésienne avec Optuna, en utilisant le \textit{Tree-structured Parzen Estimator} comme stratégie de recherche \cite{akiba2019optuna}. Dans l'expérience logicielle retenue, le score calibré est volontairement ramené à deux signaux observables dans les sorties d'évaluation : le score sémantique issu de pgvector et le score Neo4j issu de l'enrichissement graphe. L'objectif retenu est le NDCG@10, car cette métrique évalue à la fois la présence des offres pertinentes dans le top 10 et leur ordre dans la liste. Pour chaque essai, l'algorithme propose des poids positifs, les normalise afin que leur somme soit égale à 1, recalcule le score hybride, rerange les offres par candidat, puis mesure le NDCG@10 moyen. La solution retenue est celle qui maximise cette métrique sur l'échantillon d'évaluation.

Cette calibration est volontairement séparée de la génération \LLM{}. Elle concerne seulement le moteur de classement. Elle inclut également un garde-fou : les poids proposés par Optuna sont comparés à des solutions de frontière, par exemple un score purement sémantique ou un score purement fondé sur les compétences. Cette précaution évite de retenir artificiellement un score hybride si les données montrent qu'une composante unique classe mieux les offres selon le critère NDCG@10. La calibration est donc utilisée comme un test empirique des pondérations, non comme une justification automatique du caractère hybride du système.
...
Pour l'ablation \textit{pgvector} contre \textit{pgvector + graphe}, le protocole est différent. Un échantillon de 60 candidats est tiré aléatoirement avec la graine 42. Pour chaque candidat, pgvector retourne un pool initial de 30 offres candidates. Deux classements sont alors comparés :
...
En revanche, le MRR@10 diminue de 0.5602 à 0.4647 et la Precision@1 passe de 0.4182 à 0.3636. Cela signifie que le graphe ne place pas toujours la première offre pertinente aussi haut que le classement vectoriel initial. Ce résultat est important : le graphe améliore la couverture et la cohérence globale du classement, mais il peut dégrader le tout premier rang lorsque ses signaux structurels sont imparfaits, incomplets ou trop fortement pondérés.
...
Afin de ne pas conserver des pondérations arbitraires dans le moteur hybride, un module logiciel spécifique a été ajouté : \filevar{src/07\_evaluation/optimize\_hybrid\_weights.py}. Ce module lit les sorties détaillées de l'ablation, optimise les pondérations du score hybride avec Optuna et retient la combinaison qui maximise le NDCG@10. Les sorties sont sauvegardées dans \filevar{outputs/evaluation/hybrid\_weight\_optimization} et les figures correspondantes dans \filevar{rapport/figures/generated/evaluation}.

L'expérience a été conduite sur 550 couples candidat--offre correspondant à 55 candidats évalués. La pertinence graduée utilisée pour le NDCG@10 est \filevar{proxy\_relevance}, c'est-à-dire le proxy métier construit à partir des métadonnées normalisées. Cette décision est préférable à l'utilisation directe des verdicts du moteur, car elle limite la circularité : on ne calibre pas les poids uniquement sur une étiquette déjà produite par le score hybride. Cependant, cette pertinence reste un label faible ; elle ne remplace pas une annotation humaine.

\begin{table}[H]
\centering
\small
\caption{Résultat de l'optimisation bayésienne des pondérations du score hybride}
\label{tab:eval_optimisation_poids_hybrides}
\begin{tabular}{lrr}
\toprule
Configuration testée & NDCG@10 moyen & Lecture statistique \\
\midrule
Score sémantique seul & 0.9087 & Meilleure solution observée \\
Meilleur essai Optuna & 0.9060 & Très proche, mais inférieur au bord sémantique \\
Score Neo4j seul & 0.8735 & Moins performant que le signal sémantique \\
Poids égaux & 0.8704 & Combinaison naïve défavorable \\
Score hybride actuel & 0.8829 & Inférieur au score sémantique seul \\
\bottomrule
\end{tabular}
\end{table}

Le résultat est sévère pour le score hybride actuel. Sur ce protocole, la meilleure pondération retenue est :
\[
w_{semantique}=1{,}0000 \quad ; \quad w_{Neo4j}=0{,}0000.
\]
Autrement dit, avec les composantes actuellement exportées dans le fichier d'évaluation, le NDCG@10 est maximisé par le score sémantique seul. Ce résultat ne doit pas être maquillé : il indique que le signal Neo4j disponible dans cette expérience, exporté sous la forme \filevar{taux\_match}, n'améliore pas le classement selon le proxy de pertinence. Il peut rester utile pour expliquer le \textit{skill gap}, produire les verdicts ou filtrer des profils, mais il ne doit pas être surpondéré dans le classement si l'objectif unique est le NDCG@10.

\begin{figure}[H]
\centering
\includegraphics[width=0.82\textwidth]{figures/generated/evaluation/hybrid_weights_optimization_trials.png}
\caption{Évolution du NDCG@10 au cours des essais Optuna}
\label{fig:eval_hybrid_weights_trials}
\end{figure}

La figure \ref{fig:eval_hybrid_weights_trials} montre que les meilleurs essais apparaissent lorsque le poids de la composante Neo4j devient presque nul. Cette concentration des bons essais près de la frontière confirme que le problème n'est pas seulement un mauvais choix initial de pondérations : dans l'échantillon disponible, le signal graphe agrégé par \filevar{taux\_match} n'apporte pas d'amélioration marginale au NDCG@10.

\begin{figure}[H]
\centering
\includegraphics[width=0.74\textwidth]{figures/generated/evaluation/hybrid_weights_best.png}
\caption{Pondérations retenues après optimisation NDCG@10}
\label{fig:eval_hybrid_weights_best}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.80\textwidth]{figures/generated/evaluation/hybrid_weights_ndcg_comparison.png}
\caption{Comparaison du NDCG@10 avant et après calibration}
\label{fig:eval_hybrid_weights_comparison}
\end{figure}

La figure \ref{fig:eval_hybrid_weights_comparison} confirme que la calibration améliore le score hybride actuel, qui passe de 0.8829 à 0.9087. Mais cette amélioration vient d'un choix radical : revenir au score sémantique pur. La conclusion d'ingénierie est donc claire. Le moteur hybride ne doit pas être abandonné, mais le score Neo4j doit être affiné avant d'être utilisé comme pondération de ranking : distinguer compétences essentielles et secondaires, intégrer explicitement le niveau \NCF{}, exporter le score sectoriel, ajouter une composante géographique mesurable et calibrer ces signaux sur un jeu annoté. Dans l'état actuel des sorties disponibles, la stratégie optimale pour le NDCG@10 est de conserver Neo4j pour l'explication et le \textit{skill gap}, mais de ne pas laisser le signal graphe brut dominer le classement.

\section{Performance opérationnelle et stabilité}

L'évaluation d'ablation a pris 38.7 secondes pour le coeur retrieval--reranking, hors coût complet de démarrage du notebook et de chargement du modèle. Le notebook exécuté par \filevar{nbconvert} a produit les résultats en environ 85 secondes en incluant le chargement du modèle SentenceTransformer, les connexions aux bases et l'export des figures. Cette durée reste acceptable pour une évaluation hors ligne, mais elle ne représente pas encore une latence utilisateur finale, car le chatbot complet ajoute la génération \LLM{} via OpenRouter.

Le point critique est la stabilité Neo4j. Cinq candidats sur soixante ont échoué à cause d'une erreur interne \filevar{Neo.DatabaseError.Statement.ExecutionFailed} liée au page cache et à des relations corrompues. Le système a donc évalué 55 requêtes valides. Cette fragilité doit être traitée avant toute démonstration en production : il faut reconstruire ou vérifier la base Neo4j, contrôler les imports, et ajouter des tests d'intégrité sur les relations candidat--offre--compétence.
...
La deuxième sortie graphique, issue de l'ablation pgvector contre pgvector + graphe, confirme un effet plus nuancé. Le graphe améliore les métriques de couverture du top 5 et du top 10, mais il ne sécurise pas le premier rang. Cette différence est centrale pour l'interprétation métier : le graphe est utile lorsque l'utilisateur consulte une liste courte d'offres accompagnée d'explications, mais il reste insuffisamment calibré si l'objectif est de produire une unique meilleure offre.

La troisième sortie graphique, centrée sur les déplacements de rang, explique ce résultat. Neo4j ne se contente pas d'ajouter un score marginal ; il réordonne réellement le pool initial retourné par pgvector. Cette intervention peut corriger des proximités purement lexicales, mais elle peut aussi rétrograder une bonne offre lorsque les relations de compétences, de niveau ou de secteur sont incomplètes. Le gain du graphe dépend donc de la qualité des relations et du réglage des poids du score hybride.
...
La figure \ref{fig:eval_operational_summary} complète cette lecture par la stabilité d'exécution. Sur 60 requêtes demandées, 55 ont été évaluées et 5 ont échoué à cause de Neo4j. Le taux d'erreur observé atteint donc 8.3\%. Ce résultat n'annule pas les gains mesurés sur Recall@10 et NDCG@10, mais il empêche de conclure à une robustesse de production. Dans l'état actuel, la performance algorithmique du graphe existe, mais elle est conditionnée par un assainissement de la base Neo4j.
