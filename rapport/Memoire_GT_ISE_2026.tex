...
\begin{spacing}{1.5}
This thesis addresses the problem of matching job offers, candidate profiles and skills in the Cameroonian labour market for KmerAI. Keyword-based matching is often insufficient because it provides limited explanations and does not fully exploit occupational and training reference frameworks. The main objective is therefore to design and evaluate a hybrid recommendation system combining semantic retrieval, a knowledge graph and skill gap analysis.

The methodology follows an end-to-end pipeline: cleaning job and candidate data, aligning entities with ESCO, MEPC and NCF reference frameworks, fine-tuning a SentenceTransformer model, indexing vectors in PostgreSQL--pgvector, building a Neo4j knowledge graph and orchestrating the workflow with LangGraph. The system uses 13,957 job offers, 41,298 candidate profiles, 72,643 entity embeddings and 142 documentary chunks extracted from reference documents.

The results show that the fine-tuned embedding model improves semantic retrieval, reaching a Recall@10 of 0.8520 and an NDCG@10 of 0.6336. Adding the graph improves the ranking of the pgvector candidate pool: NDCG@10 increases from 0.5575 to 0.7392, while MRR@10 increases from 0.6908 to 0.8093. The generation critic, calibrated at 0.46, acts as a safeguard against poorly grounded answers.

The innovative aspect of this work lies in the combination of vector memory, Neo4j relational memory, hybrid reranking and agentic orchestration. The main limitation is the lack of large-scale independent human validation. Further work should enrich the graph, maintain the ETL pipeline for new data and build an expert-annotated evaluation set.

\textbf{Keywords:} hybrid recommendation; GraphRAG; pgvector; Neo4j; skill gap.
\end{spacing}
...
les graphes de connaissances, capable de mesurer le \emph{skill gap} entre un profil 
...
\section*{Outils techniques mobilisés pour l'étude}
...
LangGraph coordonne ces étapes et conserve la trace des outils appelés \cite{langgraphdocs2026}. Si les informations récupérées sont insuffisantes, le système peut effectuer une recherche complémentaire avant de répondre.
...
Le workflow complet est reporté en annexe à la figure~\ref{fig:architecture_methodologique_workflow_agentique}. Il suit une logique simple : la requête est analysée, pgvector retrouve les éléments proches, Neo4j vérifie les relations métier--compétence--formation, puis le \LLM{} formule une réponse contrôlée par le critic\footnote{Dans ce mémoire, le \textit{critic} désigne un module de contrôle automatique qui vérifie si la réponse générée est suffisamment appuyée par le contexte récupéré. Il ne remplace pas une validation humaine ; il sert de garde-fou contre les réponses peu fondées.}. Le tableau~\ref{tab:objectifs_choix_methodologiques}, également placé en annexe, résume la correspondance entre objectifs spécifiques et choix méthodologiques.
...
\begin{figure}[H]
  ...
  \includegraphics[width=0.56\textwidth]{api1.png}
