...
La figure \ref{fig:architecture_methodologique_workflow_agentique} résume le workflow. La requête utilisateur est d'abord interprétée, puis orientée vers les sources utiles : pgvector pour la proximité sémantique, Neo4j pour les relations métier, les chunks documentaires pour les référentiels et le \LLM{} pour la formulation finale. La génération intervient donc après récupération et structuration du contexte.

Le module de critique ferme la boucle. Si la réponse est insuffisamment appuyée par les preuves récupérées, le contexte peut être élargi avant une nouvelle génération. Le tableau~\ref{tab:objectifs_choix_methodologiques}, placé en annexe, résume la correspondance entre objectifs spécifiques et choix méthodologiques.
