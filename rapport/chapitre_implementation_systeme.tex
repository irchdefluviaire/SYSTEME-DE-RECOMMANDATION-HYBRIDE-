...
La perte d'entraînement diminue de 2,445 à 0,132, tandis que la perte de test reste plus élevée (0,433) que celle d'entraînement (0,171). Cet écart signale une généralisation moins bonne sur les paires non vues, sans divergence du modèle. Le NDCG@10 atteint 0,6492 à l'époque~4 puis recule légèrement à 0,6469 : prolonger l'entraînement n'était donc pas justifié.
...
Le modèle fine-tuné obtient un NDCG@10 de 0,6232, un MRR@10 de 0,5874 et un Recall@10 de 0,7398. Il est retenu parce qu'il place plus souvent la bonne description dans les premiers résultats. TF--IDF reste un complément rapide pour les correspondances lexicales exactes.
...
La table \texttt{embeddings} stocke les entités structurées ; \texttt{doc\_chunks} contient les fragments des référentiels. Les index permettent leur recherche vectorielle ou textuelle, tandis qu'un identifiant commun assure le passage vers Neo4j.
\begin{figure}[H]
  \centering
...
Les référentiels produisent 142 chunks : 43 pour les diplômes, 61 pour le MEPC 2013 et 38 pour la NCF 2017. Le découpage suit les titres détectés ; les fragments de moins de 80 caractères sont fusionnés et la taille maximale est fixée à 3\,000 caractères. La stratégie de secours par fenêtres glissantes n'a pas été utilisée dans l'index courant.
...
Après le rappel pgvector, Neo4j vérifie les compétences, métiers, niveaux \NCF{}, secteurs et localisations reliés aux résultats. Il transforme ainsi une proximité textuelle en correspondance métier explicable.
...
Le graphe contient 90\,230 noeuds et 550\,542 relations. Sa faible densité ($6{,}76\times10^{-5}$) n'indique pas un manque d'information : elle résulte d'un schéma spécialisé où, par exemple, une offre est reliée à ses compétences et non à toutes les entités.

La projection de matching regroupe 72\,516 noeuds et 499\,080 arêtes dans une composante connexe : les candidats et les offres peuvent être reliés par des chemins de compétences. Cette connectivité garantit l'existence des chemins, pas leur pertinence ; une relation \texttt{POSSEDE} ou \texttt{REQUIERT} manquante réduit directement la qualité du score. La composition détaillée est reportée en annexe.
...
Le score Neo4j utilise la proportion de compétences requises couvertes, et non le degré brut des noeuds. Une compétence fréquente n'améliore donc pas automatiquement une recommandation ; elle ne compte que si elle est exigée par l'offre et associée au candidat. Les statistiques détaillées figurent en annexe.
...
Sur 690 couples candidat--offre, le score pgvector est continu et centré autour de 0,52, tandis que 31,30\,\% des scores Neo4j sont nuls. pgvector distingue donc des degrés de proximité entre presque tous les couples ; Neo4j écarte plus nettement ceux pour lesquels aucune compétence requise n'est couverte. Les deux scores ne sont pas interchangeables.
