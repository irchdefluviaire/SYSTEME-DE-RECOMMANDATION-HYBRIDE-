...
\begin{figure}[H]
  ...
  \begin{minipage}[t]{0.54\textwidth}
    ...
    \includegraphics[width=\textwidth]{figures/generated/implementation_deep_current/06_chunking_diagnostics.png}
  ...
  \begin{minipage}[t]{0.43\textwidth}
    ...
    \includegraphics[width=\textwidth]{figures/generated/implementation_deep_current/06_chunking_coherence_semantique.png}
    \source{Nos travaux, mesure Python de la similarité cosinus moyenne entre les phrases de chaque chunk.}
  \end{minipage}
\end{figure}

Les figures~\ref{fig:implementation_chunking_diagnostics} et~\ref{fig:implementation_chunking_coherence} évaluent deux propriétés complémentaires. La première décrit la forme des unités indexées. Les 142 chunks se répartissent entre le MEPC 2013 (61), les diplômes (43) et la NCF 2017 (38). Leur taille médiane est de 116 tokens, mais la distribution est hétérogène : de nombreux fragments contiennent moins de 100 tokens, tandis qu'un second groupe dépasse 400 tokens. Cette dispersion traduit la structure des documents sources, composés à la fois d'intitulés courts et de sections descriptives plus développées. Elle impose toutefois de surveiller les extrêmes : un fragment très court fournit peu de contexte, alors qu'un fragment très long peut contenir plusieurs informations distinctes.

La couverture documentaire affichée dans la figure~\ref{fig:implementation_chunking_diagnostics} est un indicateur approximatif, calculé comme le rapport entre le nombre de tokens conservés dans les chunks et le nombre de tokens extraits des PDF. Sa moyenne de 42,9\,\% ne signifie donc pas que 57,1\,\% des connaissances utiles ont nécessairement été perdues : le texte écarté peut inclure des en-têtes, sommaires, répétitions ou éléments sans valeur pour le retrieval. En revanche, les faibles ratios du MEPC 2013 et de la NCF 2017 signalent qu'un contrôle manuel par échantillonnage reste nécessaire pour vérifier que les sections métier importantes ont bien été conservées.

La figure~\ref{fig:implementation_chunking_coherence} examine ensuite le contenu interne des fragments. La cohérence correspond à la similarité cosinus moyenne entre les phrases d'un même chunk : plus elle est élevée, plus les phrases traitent d'un sujet commun. La NCF 2017 présente la cohérence moyenne la plus forte (0,336), devant le MEPC 2013 (0,307) et les diplômes (0,268). Les chunks issus des diplômes sont donc les plus variables et comportent notamment une valeur très faible proche de 0,07, signe d'un fragment susceptible de réunir des phrases peu liées. La moyenne globale de 0,303 ne démontre pas, à elle seule, une bonne qualité absolue ; elle montre surtout que le découpage structurel préserve mieux l'unité thématique de la NCF que celle des documents de diplômes. Pour le GraphRAG, ces résultats justifient le maintien des métadonnées de source, la citation du passage récupéré et une vigilance particulière sur les chunks très courts, très longs ou faiblement cohérents. Les mesures détaillées sont reportées en annexe aux tableaux~\ref{tab:implementation_chunking_coverage_duplication}, \ref{tab:implementation_pgvector_chunking} et \ref{tab:implementation_chunking_coherence}.
