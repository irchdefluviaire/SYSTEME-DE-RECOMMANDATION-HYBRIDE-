...
Le graphe relie les candidats, offres, compétences, métiers, référentiels MEPC, niveaux et domaines NCF, secteurs, employeurs, localisations et chunks documentaires. Sa fonction principale n'est pas de remplacer l'embedding, mais de donner un contexte relationnel aux résultats vectoriels. Une offre proche vectoriellement peut ainsi être expliquée par ses compétences requises, son secteur, son niveau NCF ou son rattachement métier. Cette section ne décrit donc pas Neo4j en général ; elle interprète le graphe effectivement construit dans le projet.
...
Le graphe contient 90\,230 noeuds et 550\,542 relations. Sa densité orientée globale est très faible, environ $6,76 \times 10^{-5}$, ce qui est normal pour un graphe hétérogène de connaissances : toutes les entités n'ont pas vocation à être reliées entre elles. Le degré moyen non orienté vaut 12,20 relations par noeud. Cette moyenne est cependant trompeuse, car le graphe est fortement déséquilibré : les cinq premiers types de noeuds concentrent 96,21\,\% des noeuds, et les trois premières relations concentrent 73,74\,\% des liens.

\begin{table}[H]
  \centering
  \caption{Indicateurs structuraux du graphe Neo4j}
  ...
  \begin{tabularx}{0.94\textwidth}{Xr}
    ...
    \tabheader{Indicateur} & \tabheader{Valeur observée} \\
    \midrule
    Nombre total de noeuds & 90\,230 \\
    Nombre total de relations & 550\,542 \\
    Types de noeuds observés & 18 \\
    Types de relations observés & 18 \\
    Degré moyen non orienté & 12,20 \\
    Densité orientée globale & 0,0000676 \\
    Part des 5 premiers labels & 96,21\,\% \\
    Part des 3 premières relations & 73,74\,\% \\
    Noeuds isolés observés & 12\,487 \\
    \bottomrule
  \end{tabularx}
  \tableStyleEnd
  \source{\texttt{08\_neo4j\_node\_counts.csv}, \texttt{08\_neo4j\_relation\_counts.csv} et requêtes Neo4j de diagnostic}
\end{table}

Les principaux labels sont \texttt{Candidat} avec 41\,298 noeuds, \texttt{GroupeCompétences} avec 14\,579 noeuds, \texttt{OffreEmploi} avec 13\,957 noeuds, \texttt{Compétence} avec 13\,939 noeuds et \texttt{Métier} avec 3\,039 noeuds. Cette structure est cohérente avec l'objectif métier : le graphe est d'abord un graphe d'appariement, non un graphe encyclopédique. Les candidats représentent 45,77\,\% des noeuds, les offres 15,47\,\%, les compétences 15,45\,\% et les groupes de compétences 16,16\,\%. Le risque statistique est clair : si le score de recommandation n'est pas normalisé, les familles très représentées peuvent dominer la recherche au détriment de signaux plus spécifiques comme le métier, le niveau ou le secteur.
...
Les relations dominantes sont \texttt{POSSEDE}, \texttt{NECESSITE} et \texttt{REQUIERT}. Elles forment le coeur du système : candidat--compétence, métier--compétence et offre--compétence. Les relations \texttt{DANS\_SECTEUR}, \texttt{PUBLIEE\_PAR}, \texttt{LOCALISEE\_A}, \texttt{A\_NIVEAU} et \texttt{REQUIERT\_NIVEAU} ajoutent des contraintes d'interprétation. Le graphe n'est donc pas seulement volumineux ; il encode les chemins nécessaires à l'explication d'une recommandation.
...
Les degrés moyens montrent une forte hétérogénéité. Les noeuds \texttt{NiveauFormationNCF} ont un degré moyen de 5\,812,56 et un degré médian de 7\,073, parce que neuf niveaux seulement absorbent une grande partie des relations candidat--niveau et offre--niveau. Les \texttt{GroupeBaseMEPC} ont un degré moyen de 90,22, les \texttt{Secteur} de 72,69, les \texttt{Métier} de 48,61 et les \texttt{Localisation} de 43,96. À l'inverse, \texttt{DocChunk}, \texttt{SousGroupeMEPC} et plusieurs noeuds référentiels restent faiblement connectés.
...
Cette hétérogénéité est une force et une faiblesse. Elle est une force car les hubs facilitent l'explication : le système peut rattacher rapidement une offre à un niveau, un secteur, une ville ou une famille MEPC. Elle est une faiblesse car certains hubs sont trop génériques. Les cinq plus grands hubs sont des niveaux NCF, puis apparaissent des compétences très larges liées au transport, à l'assurance ou au bâtiment. Le noeud \texttt{Employeur non précisé} atteint aussi un degré de 5\,933. Ce dernier n'est pas un vrai signal métier ; il doit être pénalisé dans le reranking, sinon le graphe risque de récompenser une absence d'information.
...
La matrice des liaisons confirme que le graphe n'est pas seulement une collection de tables importées. Les relations structurantes relient candidats et compétences, offres et compétences, métiers et compétences, offres et secteurs, offres et employeurs, ainsi que les référentiels entre eux. Les motifs les plus importants sont les suivants : 221\,413 chemins \texttt{Candidat--POSSEDE--Compétence}, 58\,519 chemins \texttt{OffreEmploi--REQUIERT--Compétence} et 126\,051 chemins \texttt{Métier--NECESSITE--Compétence}. En croisant les chemins, le graphe expose 72\,832\,868 triplets candidat--compétence--offre, 27\,984\,337 triplets candidat--niveau--offre et 963\,063 triplets métier--compétence--offre. Ces chiffres expliquent pourquoi un simple top-$K$ vectoriel est insuffisant : la base relationnelle offre un espace de justification beaucoup plus riche que la similarité textuelle seule.

\begin{table}[H]
  \centering
  \caption{Chemins métiers exploitables pour l'explication}
  \label{tab:implementation_neo4j_chemins}
  \tableStyleStart
  \begin{tabularx}{\textwidth}{XrX}
    \rowcolor{darkblue}
    \tabheader{Chemin relationnel} & \tabheader{Occurrences} & \tabheader{Usage dans le système} \\
    \midrule
    Candidat--Compétence--Offre & 72\,832\,868 & Mesure des compétences communes et base du \textit{skill gap}. \\
    Candidat--Niveau NCF--Offre & 27\,984\,337 & Contrôle de cohérence entre niveau acquis et niveau requis. \\
    Métier--Compétence--Offre & 963\,063 & Rattachement métier des offres et enrichissement MEPC/ISCO. \\
    Offre--Secteur / Employeur / Localisation & 41\,607 & Filtrage métier, géographique et contextualisation de la recommandation. \\
    DocChunk--Référentiel & 142 & Citation et justification documentaire dans le GraphRAG. \\
    \bottomrule
  \end{tabularx}
  \tableStyleEnd
  \source{Requêtes Cypher de diagnostic sur Neo4j et \texttt{08\_neo4j\_patterns\_top.csv}}
\end{table}

Les communautés algorithmiques, de type Louvain, et les scores PageRank ne sont pas revendiqués dans ce chapitre. Le diagnostic d'exécution montre que les procédures GDS nécessaires n'étaient pas disponibles dans l'instance utilisée. C'est une limite méthodologique à assumer : parler de communautés sans calcul effectif serait fragile. En revanche, les composantes relationnelles métier sont déjà visibles par motifs : candidat--compétence--offre, métier--compétence--offre, offre--niveau--candidat et chunk--référentiel.
...
Sur les 550 couples candidat--offre analysés dans le fichier d'ablation, les verdicts sont très majoritairement prudents : 259 couples, soit 47,09\,\%, relèvent du vivier à développer ; 144, soit 26,18\,\%, sont hors cible dans l'état actuel ; 118, soit 21,45\,\%, nécessitent une montée en compétence ; seuls 29 couples, soit 5,27\,\%, sont directement prêts à postuler. Cette distribution est rassurante : le moteur ne transforme pas mécaniquement toute proximité vectorielle en recommandation forte.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{Xrrrr}
    ...
    \midrule
    Vivier à développer & 259 & 47,09\,\% & 0,4843 & 0,3050 \\
    Hors cible actuel & 144 & 26,18\,\% & 0,3073 & 0,2252 \\
    Montée en compétence & 118 & 21,45\,\% & 0,6870 & 0,9641 \\
    Prêt à postuler & 29 & 5,27\,\% & 0,7593 & 0,9712 \\
    \bottomrule
  \end{tabularx}
  \tableStyleEnd
...
La corrélation entre le score sémantique et le score hybride vaut 0,6002, tandis que la corrélation entre le taux de match des compétences et le score hybride vaut 0,8023. Cela montre que le moteur ne se contente pas de recopier la similarité vectorielle : la compétence pèse fortement dans la décision finale. C'est cohérent avec l'objectif emploi--compétences. En revanche, cette forte dépendance aux compétences exige de surveiller la qualité des compétences extraites : si certaines compétences sont trop génériques ou mal typées, elles peuvent influencer fortement le score.

\subsection{Effet du reranking hybride}

Le reranking hybride modifie fortement les rangs. Sur les 550 couples étudiés, 82,73\,\% gagnent des positions après intégration du graphe et des règles métier, 10,18\,\% perdent des positions et 7,09\,\% restent au même rang. Le déplacement moyen est de 8,11 rangs. Ce résultat doit être lu avec nuance : un fort déplacement de rang n'est pas automatiquement une amélioration, mais il prouve que le moteur hybride apporte une décision supplémentaire par rapport au retrieval vectoriel.
...
\begin{table}[H]
  ...
  \begin{tabularx}{0.94\textwidth}{Xrrrr}
    ...
    \midrule
    Score sémantique & 0,4963 & 0,5087 & 0,5890 & 0,7661 \\
    Score hybride & 0,4959 & 0,4863 & 0,6225 & 0,7836 \\
    Taux de match compétences & 0,4606 & 0,2750 & 0,8330 & 1,0000 \\
    Rang pgvector initial & 13,6073 & 12,0 & 21,0 & 30,0 \\
    Rang après graphe & 5,5000 & 5,5 & 8,0 & 10,0 \\
    Déplacement de rang & 8,1073 & 7,0 & 14,0 & 28,0 \\
    \bottomrule
  \end{tabularx}
  \tableStyleEnd
  \source{\texttt{outputs/evaluation/pgvector\_vs\_graph/ablation\_graph\_rerank\_details.csv}}
...
L'ablation confirme l'intérêt du moteur hybride : l'ajout du graphe augmente le recall@10 de 0,4599 à 0,5557 et le NDCG@10 de 0,3889 à 0,4681. En contrepartie, la précision@1 baisse de 0,4182 à 0,3636. Ce compromis est cohérent avec la finalité du système : pour explorer des opportunités et construire un plan de montée en compétence, élargir correctement le top 10 est plus utile que forcer une seule réponse immédiate. Si l'usage cible devient le recrutement automatique d'un seul candidat, les poids du score devront être recalibrés pour regagner de la précision@1.

\subsection{Exemple de sortie intermédiaire}
