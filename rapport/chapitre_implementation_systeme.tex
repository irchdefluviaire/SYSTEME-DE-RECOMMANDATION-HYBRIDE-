...
La base pgvector constitue le premier étage du retrieval de l'Agentic RAG. Son rôle est précis : transformer une requête, un profil ou un fragment documentaire en vecteur, récupérer rapidement un top-$K$ d'objets proches, puis transmettre ces résultats au moteur hybride et au graphe Neo4j. Elle ne décide donc pas seule de la recommandation finale ; elle construit le vivier initial sur lequel le graphe pourra ensuite vérifier les relations métier.

\subsection{Architecture physique de la mémoire vectorielle}
...
Cette séparation évite une confusion fréquente : pgvector n'est pas seulement une colonne \texttt{vector}. C'est une couche de retrieval qui associe trois informations dans la même base : le vecteur, les métadonnées métier et le lien vers Neo4j. Le système peut donc partir d'un voisin sémantique retrouvé dans PostgreSQL, puis retrouver le noeud du graphe correspondant pour obtenir les compétences, niveaux, secteurs ou chemins explicatifs.

\subsection{Tables et volumes effectivement indexés}
...
La stratégie d'ingestion est structurelle par défaut. Elle détecte les titres de section à partir de la mise en forme et des motifs numérotés, puis chaque titre ouvre un chunk dont le contenu s'étend jusqu'au titre suivant. Les fragments de moins de 80 caractères sont fusionnés avec le fragment précédent, afin d'éviter des passages sans contexte. Une limite supérieure de 3\,000 caractères contrôle la taille maximale envoyée à l'encodeur. Le pipeline prévoit une stratégie sémantique de secours par fenêtres glissantes de 800 caractères avec 120 caractères de recouvrement, mais elle n'a pas été nécessaire dans l'index courant : les 142 chunks ont été produits par la stratégie structurelle.
...
La figure \ref{fig:implementation_chunking_diagnostics} montre que les chunks sont courts mais hétérogènes. La taille médiane de 116 tokens convient au retrieval, tandis que le coefficient de variation de 0,91 signale des écarts importants entre fragments. Cette variabilité n'est pas automatiquement un défaut : elle reflète la structure inégale des référentiels. Elle impose cependant de contrôler la qualité des passages récupérés dans les réponses RAG. La couverture documentaire et la duplication des chunks sont détaillées en annexe (tableau~\ref{tab:implementation_chunking_coverage_duplication}), ainsi que les statistiques de chunking (tableau~\ref{tab:implementation_pgvector_chunking}).
...
La cohérence sémantique moyenne vaut 0,303. Ce niveau est modéré : les chunks ne sont pas incohérents, mais ils ne forment pas non plus des blocs fortement homogènes. C'est cohérent avec des référentiels administratifs qui mêlent intitulés, codes, descriptions et listes. La mesure détaillée est reportée en annexe (tableau~\ref{tab:implementation_chunking_coherence}). Pour une version de production, ce résultat justifie de tester une variante de chunking sémantique contrôlée par la cohérence intra-chunk, sans perdre la structure réglementaire.

\subsection{Index, latence et types de recherche}

Les indicateurs techniques pgvector sont reportés en annexe (tableau~\ref{tab:implementation_pgvector_indicateurs}) et le détail des index pgvector et textuels dans le tableau~\ref{tab:implementation_pgvector_index}. Dans le corps du chapitre, le point important est leur rôle : les index HNSW accélèrent la similarité vectorielle, les index par type d'entité évitent de mélanger candidats, offres, métiers et compétences, les index textuels complètent la recherche dense par des correspondances lexicales, et les identifiants Neo4j assurent la continuité avec le graphe.

La latence observée est compatible avec un retrieval interactif : la médiane vaut 30,89 ms pour un top-10 HNSW. Cette valeur laisse de la marge pour les étapes suivantes, notamment le reranking graphe et la construction du contexte. Le maximum observé, 160,40 ms, rappelle toutefois qu'il faut raisonner en distribution et non seulement en moyenne. Pour un Agentic RAG, la stabilité des temps élevés est plus importante que le meilleur cas.

La brique supporte trois types de recherche. La première est la recherche sémantique top-$K$, fondée sur la proximité cosinus entre embeddings. La deuxième est la recherche lexicale française, via les index GIN et \texttt{to\_tsvector}. La troisième est la recherche hybride : présélection vectorielle, filtre par nature d'entité, puis enrichissement par Neo4j. Cette organisation confirme que pgvector est prêt pour le retrieval de l'Agentic RAG : il produit le vivier initial, filtre les objets utiles et renvoie des identifiants directement raccordables au graphe.

La conclusion technique est donc circonscrite. pgvector indexe les entités utiles, conserve les liens Neo4j, dispose d'index vectoriels et lexicaux, et présente des latences compatibles avec une application interactive. Le point à évaluer ensuite n'est plus la disponibilité du stockage, mais la qualité du ranking produit par ce premier étage et son amélioration par le graphe.
